...
static void print_hanoi(const ulong *f, ulong n, ulong s, int md) {
  // print the towers:
...
void hanoi(ulong n) {
  ...
  for (ulong k = 1; k < n2; ++k) {
    ...
    int rp = 3;
    while (rp--)
      if (f[rp] & s) break;  // pile where disk is removed

    ulong j = 3;
    while (j--) f[j] ^= s;  // change all piles
    f[u] ^= s;              // undo change for untouched pile

    int ap = 3;
    while (ap--)
      if (f[ap] & s) break;  // pile where disk was added
