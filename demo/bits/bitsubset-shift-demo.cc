...
void show_subsets(ulong v) {
  ...
  S.prev();
  S.prev();
  print_bin("U ", S.current(), 16);
  cout << endl;

  do {
    print_bin("  ", S.shift_left(), 16);
    cout << endl;
  } while (S.current());
  cout << endl;
}
...
int main(int argc, char **argv) {
  ...
  NXARG(w,
        "With benchmark: shift until result zero, for all words w, w+1, ..., "
        "w+d");
  ulong d = 1;
  NXARG(d, "");
...
arg 1: 4294967295 == w  [With benchmark: shift until result zero, for all words
w, w+1, ..., w+d]  default=0 arg 2: 268435455 == d  []  default=1 ct=4026531843
...
/// make-target2: "1demo DSRC=demo/bits/bitsubset-shift-demo.cc
