...
int main(int argc, char **argv) {
  ...
  for (ulong k = 1; k <= n; ++k)  // note start with k==1
  {
    ...
    jjassert(bit_paper_fold_alt_rot(k) ==
             bit_paper_fold_general_rot(k, 0xaaaaaaaa));
    jjassert(0 == (bit_paper_fold_general_rot(k, w) +
                   bit_paper_fold_general_rot(k, ~w)) %
                      4);
  }
  cout << endl;
...
/// make-target2: "1demo DSRC=demo/bits/bit-paper-fold-general-demo.cc
