...
void db_manip(digraph &dg, ulong mq) {
  ulong n = dg.ng_ / 2;
  ...
  switch (mq) {
    ...
    case 4:
      // different ordering:
      for (ulong k = 0; k < n; ++k)
        if (parity(k)) dg.reverse_edge_order(k);
      break;
  }
}
// -------------------------
...
//
ulong pfunc_db(const digraph_paths &dp)
...
/// make-target2: "1demo DSRC=demo/graph/graph-debruijn-demo.cc
