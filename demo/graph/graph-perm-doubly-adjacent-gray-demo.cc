...
ulong pfunc_permgray(const digraph_paths &dp)
...
int main(int argc, char **argv) {
  ...
  uint rnds = 0;
  NXARG(rnds,
        "if set, edge order is randomized and rnds is used as random seed");
  ...
  cout << "Graph has " << dg.num_nodes() << " nodes, " << dg.num_edges()
       << " edges." << endl;
...
/// make-target2: "1demo DSRC=demo/graph/graph-perm-doubly-adjacent-gray-demo.cc
