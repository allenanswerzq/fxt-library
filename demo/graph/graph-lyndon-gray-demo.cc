...
void print(const digraph_paths &dp) {
...
ulong pfunc_lyndon_gray(const digraph_paths &dp)
...
ulong pfunc_lyndon_gray_cyc(const digraph_paths &dp)
// Function to be called with each path.
{
  //    return 0;  // just count
...
int main(int argc, char **argv) {
  ...
  ulong condn = 0;
  NXARG(condn,
        "choose condition: 0==>cycles, 1==>rot0, 2==>monotonic, 3==>1&2, "
        "4==>nil");
  ...
  switch (condn) {
    case 0:
      dp.all_paths(pfunc_lyndon_gray_cyc, 0, 0, maxnp);
      break;
    case 1:
      dp.all_paths(pfunc_lyndon_gray, 0, 0, maxnp);
      break;
    case 2:
      dp.all_cond_paths(pfunc_lyndon_gray, cfunc_monotonic, 0, 0, maxnp);
      break;
...
/// make-target2: "1demo DSRC=demo/graph/graph-lyndon-gray-demo.cc
