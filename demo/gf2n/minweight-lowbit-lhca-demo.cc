...
//% Generate minimal-weight low-bit LHCA rules (LHCA:= Linear Hybrid Cellular
//Automaton).

// Output dependent on BITS_PER_LONG

void print_rules(ulong rr, ulong cc, ulong pn) {
  //    print_bin("  r=", rr, pn);
  print_bin("  c=", cc, pn);
  print_idx_seq("  r= ", rr);
  //    print_idx_seq("  c= ", cc);
...
/// make-target2: "1demo DSRC=demo/gf2n/minweight-lowbit-lhca-demo.cc
