...
double chop(double x, double eps = 1e-9) {
  if (fabs(x) < eps) x = 0;
  return x;
}
// -------------------------

Complex chop(Complex x, double eps = 1e-9) {
  return Complex(chop(x.real(), eps), chop(x.imag(), eps));
}
// -------------------------

void printit(const char *bla, Complex *f, ulong n) {
  cout << bla << endl;
  for (ulong k = 0; k < n; ++k) {
...
int main(int argc, char **argv) {
  ...
  null(f, n);
  if (d >= 0)
    f[d] = Complex(1.0, 0.0);  // delta peak
  else                         // "random"
  {
    rseed((uint)-d);
  ...
  const Complex n1 = Complex(1.0 / (double)n, 0.0);
  for (ulong j = 0; j < n; ++j) f[j] *= n1;
