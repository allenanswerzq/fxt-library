...
#define SAYFUNC() cout << "routine = " << fn << "()" << endl;
...
int main(int argc, char **argv) {
  ...
  switch (tt) {
    case 0:
      DO(fht_loc_dif2_core);
    case 1:
      DO(fht_loc_dit2_core);
    case 2:
      DO(fht_dif_core);
    case 3:
      DO(fht_dit_core);
    case 4:
      DO(compl_fht_loc_dif2_core);
    case 5:
      DO(compl_fht_loc_dit2_core);
    case 6:
      DO(compl_fht_dif_core);
    case 7:
      DO(compl_fht_dit_core);
    default:;
  }

  cout << "size=" << n << endl;
  cout << "memsize=" << (n * sizeof(double) / 1024) << " kB"
       << " =" << (n * sizeof(double) / 1024 / 1024) << " MB" << endl;
  const double mb = (double)(1024 * 1024);
  const double xf =
      (double)ldn * (double)n * (double)sizeof(double) * (double)rep;
