...
./bin 0 100  5.83s user 0.00s system 99% cpu 5.834 total
  ...
  ## up to n = 153:
  ./bin 1000 > ~/b069999-in.txt  2281.29s user 0.00s system 99% cpu 38:01.30
total N=153;x='x+O('x^(N+1)); Vec( 1/prod(k=1,N,1-x^k)/(1-x) )[N+1] ==>
573,169,704,246 573169704246 / 2281.29 == 251,248,067 per second

  time ./bin 0 1000 > ~/b111212-in.txt
  ## up to n = 300:
  ./bin 0 1000 > ~/b111212-in.txt  7239.64s user 0.28s system 99% cpu 2:00:39.98
total N=300; x='x+O('x^(N+1)); Vec( prod(k=1,N,1+x^k)/(1-x) )[N+1] ==>
2,287,746,908,056 2287746908056 / 7239.64  == 316,002,854 per second
