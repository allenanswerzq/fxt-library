...
//% Highest degree of an irreducible representation of symmetric group S_n of
//degree n. % Cf. sequence A082914: % Minimum product of hook lengths of a
//partition of n.
...
/*
 s=0;  echo $(for n in $(seq 0 20); do ./bin $n $s; done | grep md= ) | sed 's/
md=/, /g;'

 s=1;  echo $(for n in $(seq 0 20); do ./bin $n $s; done | grep md= ) | sed 's/
md=/, /g;' A000000 (not in the OEIS)
*/
