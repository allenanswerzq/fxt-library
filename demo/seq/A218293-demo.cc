...
//% standard Young tableaux with shapes corresponding to partitions into
//distinct parts. % Also OEIS sequences % A000085 (all tableaux, d=0), % A225121
//(tableaux for partitions into distinct parts with minimal difference 2, d=0),

// Cf. seq/A003040-demo.cc
// Cf. comb/partition-dist-d-asc-demo.cc for partitions where parts differ by at
// least d Cf. comb/partition-asc-demo.cc for (all) partitions
...
/*
 s=1;
 echo $(for n in $(seq 0 20); do ./bin $n $s; done | grep tbct= ) | sed 's/
 tbct=/, /g;'
*/

/*
A003040: Highest degree of an irreducible representation of symmetric group S_n
of degree n

for n in $(seq 1 13) ; do ./bin $n 0 | cut -s -d\# -f2 | cut -d= -f2 | sort -g |
tail -1; done
