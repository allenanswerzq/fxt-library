...
// Cf. comb/partition-dist-asc-demo.cc for partitions into distinct parts as
// weakly ascending compositions Cf. comb/partition-demo.cc for partitions given
// as multiplicities of parts Cf. seq/A179080-demo.cc for partitions into
// distinct parts and adjacent parts have different parity.
...
int main(int argc, char **argv) {
  ulong n = 37;
  NXARG(n,
        "compositions into distinct parts from a set, via integer partitions");
  bool fq = true;
  ...
  if (fq)
    for (ulong k = 0; k < N; ++k) S[k] = fibonacci(k + 1);
  else
    for (ulong k = 0; k < N; ++k) S[k] = 2 * k + 1;
...
// distinct Fibonacci numbers, compositions (A218396):
 echo $(for n in $(seq 1 25) ; do ./bin $n 1 | g -F 'cct=' | cut -d= -f2; done )
| sed 's/ /, /g;' 1, 1, 3, 2, 3, 8, 2, 9, 8, 8, 32, 6, 9, 32, 8, 38, 30, 32,
150, 6, 33, 32, 32, 158, 30

// cf. A076739: compositions into Fibonacci numbers

// distinct Fibonacci numbers, partitions (A000119):
 echo $(for n in $(seq 1 25) ; do ./bin $n 1 | g -F 'pct=' | cut -d= -f2; done )
| sed 's/ /, /g;' 1, 1, 2, 1, 2, 2, 1, 3, 2, 2, 3, 1, 3, 3, 2, 4, 2, 3, 3, 1, 4,
3, 3, 5, 2

// distinct odd numbers, compositions (A032021):
 echo $(for n in $(seq 1 25) ; do ./bin $n 0 | g -F 'cct=' | cut -d= -f2; done )
| sed 's/ /, /g;' 1, 0, 1, 2, 1, 2, 1, 4, 7, 4, 7, 6, 13, 6, 19, 32, 25, 32, 31,
58, 43, 82, 49, 132, 181

// distinct odd numbers, partitions (A000700):
 echo $(for n in $(seq 1 25) ; do ./bin $n 0 | g -F 'pct=' | cut -d= -f2; done )
| sed 's/ /, /g;'
