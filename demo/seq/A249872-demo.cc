...
//% Number of iterations to reach a final state for an n X n lattice of sand
//piles on a torus. % Also A293452: number of iterations for an n X k lattice.
...
// echo $(for n in $(seq 1 20); do ./bin $n; done | grep ct= ) | sed 's/ ct=/,
// /g;' for n in $(seq 1 50); do ./bin $n | grep -F 'nscan=' ;  done time ( for
// n in $(seq 1 1000); do ./binxx $n | grep -F 'ct=' ; done > tt-A249872.txt; )
...
A293452:
for n in $(seq 1 15); do for k in $(seq 1 $n); do ./bin $n $k | g ct= | tr -d
'ct='; done ; done As triangle: for n in $(seq 1 15); do echo $(for k in $(seq 1
$n); do ./bin $n $k | g ct= | tr -d 'ct='; done) ; done
