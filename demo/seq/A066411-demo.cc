...
//% Number of sums of c_k * (n choose k) where the c_k are a permutation of 0
//through n. % Sequence A126972: permutations of [1..n], sum(k=1..n,
//(pi(k)-k)^2). % Sequence A189043: permutations of [1..n], sum(k=1..n, k^2 *
//pi(k) ).
...
int main(int argc, char **argv) {
  ...
  //    for (ulong k=0; k<n; ++k)  C[k] = k*k;  // A000000 (initial erroneous
  //    A189043) for (ulong k=0; k<n; ++k)  C[k] = (k+1)*(k+1);  // A189043

  //    for (ulong k=0; k<n; ++k)  C[k] = k%2;  // A033638
  //    for (ulong k=0; k<n; ++k)  C[k] = 1-(k%2);  // A033638

  //    for (ulong k=0; k<n; ++k)  C[k] = k/2;  // A000000: 1, 1, 3, 5, 11, 17,
  //    29, 41, 61, 81, 111, 141, ...
