...
void doit(ulong ldn, bool cq, bool miq) {
  ...
  print_bin(" za = ", L.za_, pn);
  cout << endl;
  print_bin("  v = ", L.B_.full_set(), pn);
  cout << endl;
  cout << " minima = zi XOR subsets(v) where " << endl;
  print_bin(" zi = ", L.zi_, pn);
  cout << endl;
  ...
  do {
    ...
    if (cq)  // print cycle:
    {
      ...
      //            print_bin("  diameter = ", ma-mi, pn);  cout  << " = " <<
      //            setw(4) << ma-mi;
...
int main(int argc, char **argv) {
  ulong ldn = 7;
  NXARG(ldn,
        "Cycle leaders for Gray permutation where highest bit is at position "
        "ldn");
...
/// make-target2: "1demo DSRC=demo/perm/permgray-leaders-demo.cc
