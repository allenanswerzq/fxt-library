...
#else
  if (q)
    for (ulong k = 0; k < kk; ++k) bitrev(f, 1, 0, n, 1);
  else
    for (ulong k = 0; k < kk; ++k) revbin_permute(f, n);
#endif
...
//./bin 20 10 0  0.87s user 0.01s system 100% cpu 0.876 total  //
//revbin_permute()
//./bin 20 10 1  1.38s user 0.01s system 99% cpu 1.402 total  // bitrev()

//./bin 8 1000000 0  0.86s user 0.00s system 99% cpu 0.866 total  //
//revbin_permute()
...
/// make-target2: "1demo DSRC=demo/perm/revbin-perm-rec-demo.cc
