...
// Swap arrays
{
  for (ulong k = 0; k < n; ++k) std::swap(f[k], g[k]);
}
// -------------------------
...
static void identity(ulong *, ulong)
...
static void hswap(ulong *f, ulong n)
// Swap halves.
{
  swap(f, f + n / 2, n / 2);
}
...
static void pswap(ulong *f, ulong n)
// Swap pairs.
{
...
static bool qtest(const ulong *f, ulong n)
...
static void perm_r2(const ulong *f, ulong n, const char *str, PF p1,
                    const char *pstr, PF pp, ulong *g) {
  set_seq(g, n);
  pp(g, n);
  p1(g, n);
  if (compare(f, g, n)) {
...
static void perm_r1(const ulong *f, ulong n, const char *str, PF p1) {
  ALLOCA(ulong, g, n);
  set_seq(g, n);
  p1(g, n);
  if (compare(f, g, n)) {
...
void perm_recognize(const ulong *f, ulong n)
...
void p1_func(ulong *f, ulong n) {
  ...
  //    P1( swap(f, f+n4, n4) );  // rev @ gray_rev_permute == rev @
  //    gray_permute @ rev P1( swap(f+n4, f+nh, n4) );  // unzip P2( swap(f,
  //    f+nh+n4, n4) );  // unzip @ rev

  //    P1( swap(f+n4, f+nh, n4) );  P1( swap(f+nh, f+nh+n4, n4) );  //
  //    unzip_rev P1( swap(f, f+n4, n4) );  P1( swap(f, f+nh, n4) );  // rev @
  //    unzip_rev @ rev
...
void p2_func(ulong *f, ulong n) {
  ...
  //    P2( swap(f, f+nh, nh) );  P2( swap(f, f+n4, n4) );  //
  //    inverse_gray_rev_permute

  //    P1( swap(f+nh, f+nh+n4, n4) );  P1( swap(f+n4, f+nh, n4) );  // zip_rev
  //    P1( swap(f, f+nh, n4) );  P1( swap(f, f+n4, n4) );  // rev @ zip_rev @
  //    rev

  //    P2( swap(f, f+nh, n4) );  // rev @ green_rev_permute == rev @
  //    green_permute @ rev P2( swap(f+n4, f+nh+n4, n4) );  // green_permute ==
  //    green_rev_permute @ rev
...
int main(int argc, char **argv) {
  ...
  //       reverse(f, n/2); inverse_gray_permute(f,  n/2); revbin_permute(f,
  //       n/2); reverse(f+n/2, n/2); inverse_gray_permute(f+n/2,  n/2);
  //       revbin_permute(f+n/2, n/2);
  //        );
  //// = unzip_rev @. reverse
  //// = h @. unzip_rev
  ...
  //        revbin_permute(f+n/2, n/2); gray_permute(f+n/2,  n/2);
  //        reverse(f+n/2, n/2); reverse(f, n); inverse_gray_permute(f,  n);
  //        revbin_permute(f, n);
  //        );
  // = reverse .@ zip_rev
  // = zip_rev .@ h
