  ...
  do {
    ...
    switch (sq) {
      ...
      case 6:  // position of last one: A078391
        ...
        // 0, 16796, 4862, 2860, 2145, 1848, 1764, 1848, 2145, 2860, 4862,
        // 16796, 0,
      ...
      case 70:  // min limb length: A000000  (Dyck paths with shortest pyramid
                // size k)
        s = T.min_limb_length();
      ...
      case 71:  // max limb length: A120059 (Dyck paths with longest pyramid
                // size k)
        s = T.max_limb_length();
        // columns: A086581
        break;
        ...
        // 0, 62642, 376727, 213915, 65446, 17741, 4713, 1254, 336, 91, 25, 7,
        // 2, 1, 0,
...
/*
  ...
  OEIS example:
  q=0; for n in $(seq 0 11) ; do ./bin $n $q | grep ', $' ; done | nl -v0 -s':
' -w2 -n rz

  C++ comment:
q=0;  for n in $(seq 0 11) ; do ./bin $n $q | grep ', $' ; done | sed 's/^/\/\/
/; s/ $//;'

Extract column c from stats q:
q=0;  c=0;  echo $(for n in $(seq 0 11) ; do ./bin $n $q | grep ', $' ; done |
cut -d, -f$((c+1))) | sed 's/ /, /g;'
...
/// make-target2: "1demo DSRC=demo/comb/ordered-tree-lev-seq-stats-demo.cc
