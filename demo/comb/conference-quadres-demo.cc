...
void smatprint(const char *bla, Smat &m) {
  cout << bla << endl;
...
void doit(ulong q) {
  ...
  //    for (ulong k=0; k<Q; ++k)  vec[k] = kronecker(k, q);  // Kronecker
  //    symbol
...
int main(int argc, char **argv) {
  ...
  if (1 == argc) {
    ...
    for (ulong k = 1; k < (ulong)argc; ++k) doit(strtoul(argv[k], nullptr, 10));
  }
...
/*
 for p in $(primes 3 101); do echo $p": "; ./bin $p  2>&1 >/dev/null || break;
 done
*/
...
/// make-target2: "1demo DSRC=demo/comb/conference-quadres-demo.cc
