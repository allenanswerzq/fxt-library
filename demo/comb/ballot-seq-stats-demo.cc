...
int main(int argc, char **argv) {
  ...
  do {
    ...
    switch (sq) {
      ...
      case 20:  // number of smallest parts (=number of cells in first row):
                // A047884
        s = W.num_min_val();
        break;
        // 1,
      ...
      case 21:  // number of largest parts (=number of cells in last row):
                // A238123
        s = W.num_max_val();
        break;
        // 1,
      ...
      case 61:  // abs(#minval - #maxval)  (=diff row lengths: first-last):
                // A238707
        // column 0: A067228
        ...
        //            // 1, 1, 2, 4, 6, 9, 13, 16, 19, 22, 23, 23, 22, 19, 16,
        //            13, 9, 6, 4, 2, 1, 1,
        //            // 1, 1, 2, 4, 7, 10, 16, 22, 30, 37, 46, 52, 60, 62, 64,
        //            //    62, 60, 52, 46, 37, 30, 22, 16, 10, 7, 4, 2, 1, 1,

      case 80:  // tableaux by number of descents: A161126 (same as number of
                // ascents)
...
OEIS example, offset 1:
 q=30; for n in $(seq 1 10) ; do ./bin $n $q | grep ', $' ; done | nl -v1 -s':
' -w2 -n rz | sed 's/ 0, $//g;'
...
C++ comment, offset 1:
 q=30; for n in $(seq 1 10) ; do ./bin $n $q | grep ', $' ; done | sed 's/^/\/\/
/' | sed 's/ 0, $//g;'
...
/// make-target2: "1demo DSRC=demo/comb/ballot-seq-stats-demo.cc
