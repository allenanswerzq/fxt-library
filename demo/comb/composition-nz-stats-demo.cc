...
OEIS example:
 for n in $(seq 0 15) ; do ./bin $n $q | grep ', $' ; done | nl -v0 -s':  ' -w2
-n rz
...
Extract column c from stats q:
 c=0;  echo $(for n in $(seq 0 20) ; do ./bin $n $q | grep ', $' ; done | cut
-d, -f$((c+1))) | sed 's/ /, /g;'
...
/// make-target2: "1demo DSRC=demo/comb/composition-nz-stats-demo.cc
