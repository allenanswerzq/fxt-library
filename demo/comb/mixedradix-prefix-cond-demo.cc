...
bool cond_true(const ulong *, ulong) { return true; }
// -------------------------

bool cond_false(const ulong *, ulong) { return false; }
// -------------------------
...
// Base 3 gives A132109
{
  ...
  //        if ( a[j] > ak1 )  return false;  // A006318 cf. class
  //        schroeder_rgs_lex
  return true;
}
// -------------------------
...
// Total ascent: A000000
{
  ...
  //    if ( k==0 )  return true;
  ulong s = 0;
  for (ulong j = 1; j < k; ++j) s += (a[j] > a[j - 1] ? a[j] - a[j - 1] : 0);
  if (a[k] > s + 1) return false;
  return true;
}
// -------------------------
...
// A187761, cf. class map23_rgs
{
  ...
  //    ulong z = d; // a[k] is either a fixed point or a digit from the prefix
  //    (==> A000110)
  ulong z = a[d];  // z=f(f(x))
  return (z == a[z]);
  //    return true;
...
// s>=2: A000085 (involutions)
{
  ...
  ulong s = 0;
  for (ulong j = 0; j <= k; ++j) s += (d == a[j]);
  if (s >= 2) return false;  // A000085, e.g.f. exp(x+x^2/2)
  //    if ( s >= 3 )  return false;  // A001680, e.g.f. exp(x+x^2/2+x^3/6)
  //    if ( s >= 4 )  return false;  // A001681, e.g.f.
  //    exp(x+x^2/2+x^3/6+x^4/24) if ( s >= 5 )  return false;  // A110038,
...
bool cond_perm_ncyc(const ulong *a, ulong k) {
  ...
  //    if ( d == z-1 )  return false;  // A000774 permutations into at most two
  //    cycles if ( d == z-2 )  return false;  // A000000 if ( d == z+1 ) return
  //    false;  // all permutations
...
int main(int argc, char **argv) {
  ...
  ulong rr = 1;
  NXARG(rr,
        "Base (radix) of digits (0==>falling factorial, 1==>rising factorial)");
  ...
  switch (c) {
    ...
    case 11:
      M.first(cond_dist_smooth);
      break;
    case 12:
      M.first(cond_dist2);
      break;
    case 13:
      M.first(cond_sorted_full);
      break;
    case 14:
      M.first(cond_ccf_map);
      break;
    ...
    case 15:
      ...
      break;
    case 16:
      M.first(cond_aj_neq_akp1);
      break;
    case 17:
      M.first(cond_descent);
      break;
    case 18:
      M.first(cond_change);
      break;
    ...
    case 20:
      M.first(cond_invol_fp);
      break;
    case 21:
      M.first(cond_zero_map_s);
      break;
    ...
    case 30:
      M.first(cond_arrange);
      break;
    case 31:
      M.first(cond_arrange1);
      break;
    case 32:
      M.first(cond_arrange2);
      break;
    ...
    case 40:
      ...
      break;
    case 41:
      M.first(cond_perm_ncyc);
      break;

    case 50:
      M.first(cond_sorted_desc);
      break;

    case 60:
      M.first(cond_pref_sum);
      break;
    ...
    case 90:
      M.first(cond_setpart0);
      break;
    case 91:
      M.first(cond_setpart);
      break;
    case 92:
      M.first(cond_setpart2);
      break;
    case 93:
      M.first(cond_setpart3);
      break;
    ...
    case 130:
      M.first(cond_pref_fixedpoints);
      break;

    case 150:
      M.first(cond_total_ascent);
      break;
...
arg 2: 4 == rr  [Base (radix) of digits (0==>falling factorial, 1==>rising
factorial)]  default=1 arg 3: 4 == c  [Condition function: ... ] ct=331160282
./bin 20 4  6.21s user 0.00s system 99% cpu 6.217 total
 ==> 331160282/6.21 == 53,326,937 per second

*/

// % echo $(for n in $(seq 0 11); do ./bin $n 1 150 | grep ct= | sed 's/
// ct=//;s/$/, /;' ; done)
...
/// make-target2: "1demo DSRC=demo/comb/mixedradix-prefix-cond-demo.cc
