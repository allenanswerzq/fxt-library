...
// permutation of x[0, ...,n-1].
{
  ...
  for (ulong k = n - 1, j = 2; k != 0; --k, ++j)
    rotate_right(x + k - 1, j, fc[k - 1]);

  //    for (ulong k=n-1, j=2;  k!=0;  --k, ++j)  rotate_left(x+k-1, j,
  //    fc[k-1]); reverse(x, n); make_complement(x, x, n);
}
// -------------------------
...
/// make-target2: "1demo DSRC=demo/comb/perm-rot-unrank-demo.cc
