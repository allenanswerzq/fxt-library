...
void sl_gray_rec(ulong n, bool z = true) {
  if (n == N) {
    visit(n);
    return;
  }
  ...
  // as is, md == 2 (i.e., two-close transitions) for all even radices >= 4 (and
  // md==3 for rad. 2)
  //    z = !z;  // md == 2 for odd radices (but only modular Gray code in last
  //    track) if ( (n%2) == 0 )  z = !z;  // md == 3 for all rad. >= 3 (md==5
  //    for rad. 2) z ^= (k&1);  // md == 2 for ALL radices >= 3 (but for odd
  //    rad. only modular Gray code in last track)
  ...
  } else  // backward:
  {
    ...
    dir[n] = -1UL;
    for (ulong j = k - 1; j > 0;
         --j)  // falling nonzero values followed by zeros
    {
      rv[n] = j;
      visit(n);
    }
...
/// make-target2: "1demo DSRC=demo/comb/mixedradix-sl-gray-rec-demo.cc
