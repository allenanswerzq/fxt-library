...
// Cf. comb/composition-dist-unimodal-demo.cc for unimodal compositions into
// distinct parts Cf. comb/partition-asc-2rep-demo.cc for partitions with
// mulitplicities of parts <= 2
...
int main(int argc, char **argv) {
  ...
  ulong m = P.num_parts();
  do {
#if 0  // due to the internal representation the following is slightly
       // nontrivial:
...
//            if ( x[j] - x[j-1] < 2 )   // min diff
    ...
    //        cout << m << ", " << endl;  return(0); // first has max number of
    //        parts
    //  echo  $(for n in $(seq 0 40) ; do ./bin $n ; done | g -v arg)
    // 0, 1, 1, 2, 3, 3, 3, 4, 4, 5, 5, 5, 5, 6, 6, 6, 7, 7, 7, 7, 7, 8, 8, 8,
    // 8, 9, 9, 9, 9, 9, 9, 10, ...
    // == A293137, cf. A293136
...
/// make-target2: "1demo DSRC=demo/comb/composition-unimodal-demo.cc
