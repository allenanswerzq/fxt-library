...
//d(k)>=0, %   and d(k) <= 1 + asc([d(1), d(2), ..., d(k-1)]) and asc(.) counts
//the number %   of ascents of its argument. % A137251: ascent sequences with k
//ascents. % A175579: ascent sequences with k zeros. % A218579: ascent sequences
//with position of last zero at index k-1. % A218580: ascent sequences with
//position of first occurrence of maximal value at index k-1. % A218581: ascent
//sequences with position of last occurrence of maximal value at index k-1.
  ...
  do {
    ...
    switch (sq) {
      ...
      case 2:  // by number of descents: A238858
        ...
        // 2048, 163835, 1728458, 4537169, 3574869, 834115, 45747, 262, 0, 0, 0,
        // 0, 0, 4096, 502769, 7689744, 29532566, 35900857, 14475124, 1752513,
        // 45355, 76, 0, 0, 0, 0, 0,
      ...
      case 5:  // by number of max digits: A000000
        ...
        // 7669233, 2353714, 647597, 165008, 39571, 8969, 1924, 396, 77, 12, 1,
        // 1,
      ...
      case 11:  // number of odd values: row reversal of "even values"
        ...
        // 1, 4357, 44934, 175754, 355046, 414118, 287276, 114995, 23750, 1842,
        // 1, 0,
      ...
      case 61:  // abs(#minval - #maxval): A000000
        ...
        // 268807, 506657, 376229, 186591, 63913, 16229, 3082, 509, 46, 11, 0,
        // 0,
    ...
    //        cout << setw(3) << A.num_zeros() << setw(3) <<
    //        A.num_fixed_points();
...
OEIS example:
q=0; for n in $(seq 0 13) ; do ./bin $n $q | grep ', $' ; done | nl -v0 -s':  '
-w2 -n rz

C++ comment:
q=0;  for n in $(seq 0 13) ; do ./bin $n $q | grep ', $' ; done | sed 's/^/\/\/
/; s/ $//;'

Extract column c from stats q:
q=0;  c=0;  echo $(for n in $(seq 0 13) ; do ./bin $n $q | grep ', $' ; done |
cut -d, -f$((c+1))) | sed 's/ /, /g;'
...
/// make-target2: "1demo DSRC=demo/comb/ascent-rgs-stats-demo.cc
