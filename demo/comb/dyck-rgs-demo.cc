...
int main(int argc, char **argv) {
  ...
  do {
    ...
    //            // Number of Dyck paths of semilength n-1 with no UUU's and no
    //            DDD's, ulong dt[N];  // delta sequence for (ulong j=0; j<N;
    //            ++j)  dt[j] = 0; for (ulong j=0; j<n; ++j)  dt[ds[j]] = 1;
    //
    //            bool q = true;
    //            for (ulong j=2; j<N; ++j)
    //                if ( (dt[j]==dt[j-1]) && (dt[j-1]==dt[j-2]) )  { q=false;
    //                break; }
    //            if ( !q )  { --ct; continue; }
    //        }
