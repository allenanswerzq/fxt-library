...
void comb_enup(ulong n, ulong k, bool z) {
  if (k == n) {
...
void comb_enup_compl(ulong n, ulong k, bool z) {
  if ((k == 0) || (k == n)) {
    for (ulong j = 1; j <= k; ++j) rv[j] = j;
    ++ct;
    visit();
...
/// make-target2: "1demo DSRC=demo/comb/combination-enup-rec-demo.cc
