...
OEIS example:
q=0; for n in $(seq 0 8) ; do ./bin $n $q | grep ', $' ; done | nl -v0 -s':  '
-w2 -n rz

C++ comment:
q=0;  for n in $(seq 0 8) ; do ./bin $n $q | grep ', $' ; done | sed 's/^/\/\/
/; s/ $//;'
...
/// make-target2: "1demo DSRC=demo/comb/cayley-perm-stats-demo.cc
