...
int main(int argc, char **argv) {
  ...
  for (ulong i = 3; i < (ulong)argc; ++i)
    r[i - 3] = strtoul(argv[i], nullptr, 10);

  mixedradix_subset_lexrev M(n, rr, (argc > 3 ? r : nullptr));
  M.print_nines("Nines: ");
  cout << endl;
#endif
  ...
  do {
    ...
    //        ulong Z[64];  if ( ! is_young_tab_rgs(x, n, Z, 0) )  continue;  //
    //        strange order if ( ! is_dyck_rgs(x, n) )  continue;  // strange
    //        order
...
/// make-target2: "1demo DSRC=demo/comb/mixedradix-subset-lexrev-demo.cc
