...
// A001227 (odd divisors of n)).
{
  ...
  //    return  ( a[k] == a[k-1] + 2 );  // A038548 Number of divisors <=
  //    sqrt(n).
}
// -------------------------
...
// superdiagonal compositions and partitions
{
  ...
  //    if ( (k!=0) && (a[k] <= a[k-1]) )  return false;  // partitions into
  //    distinct parts

  //    return ( a[k] > k+1 );
  // strictly superdiagonal:
  // compositions: A238874; partitions: A238394; dist. part: A025147 (parts >=
  // 2)
...
bool cond_superdiag_growth(const ulong *a, ulong k) {
  //    if ( (k!=0) && (a[k] < a[k-1]) )  return false;  // partitions
  //    if ( (k!=0) && (a[k] <= a[k-1]) )  return false;  // partitions into
  //    distinct parts if ( k==0 )  return true;
...
bool cond_subdiag_growth(const ulong *a, ulong k) {
  //    if ( (k!=0) && (a[k] < a[k-1]) )  return false;  // partitions
  //    if ( (k!=0) && (a[k] <= a[k-1]) )  return false;  // partitions into
  //    distinct parts if ( k==0 )  return true; return  ( a[k] - a[0] <= k );
  //    // faulty: gives A177510
  return ((long)(a[k] - a[0]) <= (long)k);
  // subdiagonal growth:
...
bool cond_test(const ulong *a, ulong k) {
  ...
  //    if ( a[k] > a[k-1] )  return false;  // A027187 (even), cf. A027193
  //    (odd) return true;
...
//  echo $(for n in $(seq 1 20); do ./bin $n 99; done | grep ct= ) | sed 's/
//  ct=/, /g;'
...
int main(int argc, char **argv) {
  ...
  switch (c) {
    ...
    case 33:
      P.first(cond_no_3prog);
      break;

    case 40:
      P.first(cond_par_diff);
      break;

    case 45:
      P.first(cond_first_max);
      break;
    case 46:
      P.first(cond_first_min);
      break;
    case 47:
      P.first(cond_min);
      break;
    ...
    case 65:
      ...
      break;
    case 66:
      P.first(cond_diff_growth);
      break;

    case 70:
      P.first(cond_superdiag);
      break;
    case 71:
      P.first(cond_subdiag);
      break;
    case 72:
      P.first(cond_superdiag_growth);
      break;
    case 73:
      P.first(cond_subdiag_growth);
      break;
    ...
    case 91:
      P.first(cond_wiggly1);
      break;
    case 92:
      P.first(cond_wiggly2);
      break;

    case 95:
      P.first(cond_diff_neq1);
      break;
...
condition q for OEIS sequence:
 echo $(for n in $(seq 1 30) ; do ./bin $n $q | grep ct=; done) | sed 's/ct=//g;
s/ /, /g;'
...
/// make-target2: "1demo DSRC=demo/comb/composition-nz-prefix-cond-demo.cc
