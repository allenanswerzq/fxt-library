...
void comb_emk(ulong n, ulong k, bool z) {
  if (k == n) {
...
void comb_emk_compl(ulong n, ulong k, bool z) {
  if ((k == 0) || (k == n)) {
    for (ulong j = 1; j <= k; ++j) rv[j] = j;
    ++ct;
    visit();
...
/// make-target2: "1demo DSRC=demo/comb/combination-emk-rec-demo.cc
