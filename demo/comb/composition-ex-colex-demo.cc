...
// Cf. comb/composition-nz-demo.cc  compositions into any number of nonzero
// parts
...
int main(int argc, char **argv) {
  ...
  do {
    // OEIS sequences:
    //        {  bool qq = true;
    //            for (ulong j=1; j<k; ++j)  if ( x[j]-x[j-1] != 1 )  {
    //            qq=false; break; } if ( !qq )  continue;  // A129372
    //        }
    //        {  bool qq = true;  // A106351: no two adjacent parts are equal
    //            for (ulong j=1; j<k; ++j)  if ( x[j]==x[j-1] )  { qq=false;
    //            break; } if ( !qq )  continue;
    //        }
    //        {  if (k>1 ) { bool qq = false;  // A131044: at least two adjacent
    //        parts are equal.
    //            for (ulong j=1; j<k; ++j)  if ( x[j]==x[j-1] )  { qq=true;
    //            break; } if ( !qq )  continue; }
    //        }
    //        {  bool qq = true;
    //            for (ulong j=0; j<k; ++j)
...
/// make-target2: "1demo DSRC=demo/comb/composition-ex-colex-demo.cc
