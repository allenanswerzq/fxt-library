...
int main(int argc, char **argv) {
  ...
  ulong pct = 0, nct = 0,
        lct = 0;  // count pre-necklaces, necklaces and Lyndon words
...
k-ary pre-necklaces:
% echo $(for n in $(seq 1 14); do ./bin $n 2 | g 'ct:'; done | cut -d' ' -f2) |
sed 's/ /, /g;' Number of irreducible polynomials over GF(k) of degree at most
n. a(n)=sum(m=1,n, 1/m* sumdiv(m, d, moebius(d)*k^(m/d) ) ); k=2: A062692, k=3:
A114945,  k=4: A114946,  k=5: A114947, k=6: A000000, k=7: A000000, k=8: A000000,
k=9: A000000

k-ary necklaces:
Number of k-ary irreducible polynomials over GF(k) whose degree divides n.
% echo $(for n in $(seq 1 14); do ./bin $n 2 | g 'ct:'; done | cut -d' ' -f3) |
sed 's/ /, /g;' k=2: A000031, k=3: A001867,

k-ary Lyndon words:
Number of degree-n irreducible polynomials over GF(k)
% echo $(for n in $(seq 1 14); do ./bin $n 2 | g 'ct:'; done | cut -d' ' -f4) |
sed 's/ /, /g;' k=2: A001037,
