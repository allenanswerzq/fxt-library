...
int main(int argc, char **argv) {
  ...
  do {
    ...
    if (rq)
      rfact2invperm(a, n, tp);
    else
      ffact2invperm(a, n, tp);
    print_perm("    ", tp, n, dfz);
    //        print_mixedradix("    :: ", tp, n, dfz);
    for (ulong j = 0; j < n; ++j) {
      jjassert(j == p[tp[j]]);
    ...
    if (rq)
      rfact2invperm(a, n, tp, LR);
    else
      ffact2invperm(a, n, tp, LR);
    //        print_mixedradix("    :: ", tp, n, dfz);
    for (ulong j = 0; j < n; ++j) {
      jjassert(j == p[tp[j]]);
    }
  ...
  ulong n =
      10 * 1000 * 1000;  // note: ULIMIT with demo-script (kills this branch)
  NXARG(n, "Number of elements to permute");
  ...
  if (!rq)
    for (ulong k = 0, r = n - 2; k < r; ++k, --r) swap2(a[k], a[r]);
...
/// make-target2: "1demo DSRC=demo/comb/big-fact2perm-demo.cc
