...
// Primitive polynomial used is x^64 + x^4 + x^3 + x^2 + 1
{
 ...
 public:
  ...
  void word_in(Type w) {  // Fibonacci setup
    uint p = pos_;
    pos_ = (p + 1) & m_;
    uint h = (p - 1) & m_;
    Type a = x_[p & m_];  // 0
  ...
  void zero_pos() {  // move constant term to position zero
    rotate_left(x_, 64, pos_);
    pos_ = 0;
  }

  Type sum() {  // Return sum (XOR) of all CRC words
    Type s = 0;
