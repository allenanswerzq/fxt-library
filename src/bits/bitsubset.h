...
//
{
 ...
 public:
  ...
  ulong next_all_blocks() {
    U = (U - V + ~V) & V;
    return U;
  }
  ulong prev_all_blocks() {
    U = (U + V) & V;
    return U;
  }
  ulong negate_all_blocks() {
    complement();
    return next_all_blocks();
  }
  ulong set_all_blocks_one() {
    U = (-V + ~V) & V;
    return U;
  }
  ulong set_right_block_borders() {
    U |= ((-V + ~V) & V);
    return U;
  }
  ...
  ulong shift_left_fill() {
    shift_left();
    next();
    return U;
  }

  ulong shift_left_blocks() {
    U = (U << 1) & V;
    return U;
  }
  ulong shift_left_blocks_fill() {
    shift_left_blocks();
    U |= ((-V + ~V) & V);
    return U;
  }
