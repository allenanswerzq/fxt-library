...
// where 0 <= i < bit_count(x).
{
  ...
  ulong x4 = ((x2 >> 2) & 0x3333333333333333UL) +
             (x2 & 0x3333333333333333UL);              // 0-4 in 4 bits
...
#if BITS_PER_LONG == 32
  ulong x16 =
      (0x00ff00ffUL & x8) + (0x00ff00ffUL & (x8 >> 8));  // 0-16 in 16 bits
//    ulong x32 = (0x0000ffffUL & x16) + (0x0000ffffUL & (x16>>16));  // 0-32 in
//    32 bits jjassert( x32 == bit_count(x) );
#else
  ulong x16 = (0x00ff00ff00ff00ffUL & x8) +
              (0x00ff00ff00ff00ffUL & (x8 >> 8));  // 0-16 in 16 bits
  ulong x32 = (0x0000ffff0000ffffUL & x16) +
              (0x0000ffff0000ffffUL & (x16 >> 16));  // 0-32 in 32 bits

//    ulong x64 = (0x00000000ffffffffUL & x32) + (0x00000000ffffffffUL &
//    (x32>>32));  // 0-64 in 64 bits jjassert( x64 == bit_count(x) );
#endif
...
//#else
//    ulong y = x - ((x>>1) & 0x5555555555555555UL);                     // 0-2
//    in 2 bits y = ((y>>2) & 0x3333333333333333UL) + (y &
//    0x3333333333333333UL);  // 0-4 in 4 bits y = ((y>>4) + y) &
//    0x0f0f0f0f0f0f0f0fUL;                           // 0-8 in 8 bits ulong ct
//    = (y * 0x0101010101010101UL) >> 56;  // bit count
//#endif
//
//    if ( ct <= i )  return ~0UL;  // less than i bits set
//
