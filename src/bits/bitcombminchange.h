...
// Return the inverse Gray code of the previous combination in minimal-change
// order. Input must be the inverse Gray code of the current combination.
// Constant amortized time (CAT).
...
#else
  const ulong f =
      0xaaaaaaaaaaaaaaaaUL >> (BITS_PER_LONG - k);  // == first_sequency(k);
#endif
