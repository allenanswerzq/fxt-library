...
// (OEIS sequence A000120).
{
  ...
  //    0-2 in 2 bits x = (0x3333333333333333UL & x) + (0x3333333333333333UL &
  //    (x>> 2));  // 0-4 in 4 bits x = (0x0f0f0f0f0f0f0f0fUL & x) +
  //    (0x0f0f0f0f0f0f0f0fUL & (x>> 4));  // 0-8 in 8 bits x =
  //    (0x00ff00ff00ff00ffUL & x) + (0x00ff00ff00ff00ffUL & (x>> 8));  // 0-16
  //    in 16 bits x = (0x0000ffff0000ffffUL & x) + (0x0000ffff0000ffffUL &
  //    (x>>16));  // 0-32 in 32 bits x = (0x00000000ffffffffUL & x) +
  //    (0x00000000ffffffffUL & (x>>32));  // 0-64 in 64 bits return x;

  //    // VERSION 2:
  //    x = ((x>>1) & 0x5555555555555555UL) + (x & 0x5555555555555555UL);  //
  //    0-2 in 2 bits x = ((x>>2) & 0x3333333333333333UL) + (x &
  //    0x3333333333333333UL);  // 0-4 in 4 bits x = ((x>>4) + x) &
  //    0x0f0f0f0f0f0f0f0fUL;                           // 0-8 in 8 bits x +=
  //    x>> 8;                                                       // 0-16 in
  //    8 bits x +=  x>>16; // 0-32 in 8 bits x +=  x>>32; // 0-64 in 8 bits
  //    return  x & 0xff;

  // VERSION 3:
  x -= (x >> 1) & 0x5555555555555555UL;  // 0-2 in 2 bits
  x = ((x >> 2) & 0x3333333333333333UL) +
      (x & 0x3333333333333333UL);             // 0-4 in 4 bits
