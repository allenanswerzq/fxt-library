...
#define MU4 0x0000ffffUL  // MU4 == ((-1UL)/65537UL) == (16 ones)
  ...
  ulong r = (ulong)ld_neq(x, x & MU0) + ((ulong)ld_neq(x, x & MU1) << 1) +
            ((ulong)ld_neq(x, x & MU2) << 2) +
            ((ulong)ld_neq(x, x & MU3) << 3) + ((ulong)ld_neq(x, x & MU4) << 4);
#if BITS_PER_LONG > 32
...
static inline ulong highest_zero_idx(ulong x) { return highest_one_idx(~x); }
// -------------------------
