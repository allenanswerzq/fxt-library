...
extern int nextarg_act;  // nextarg.cc

template <typename Type>
void next_arg(Type &v, const char *vname, const char *what, int argc,
              char **argv) {
  const Type def = v;
  ++nextarg_act;
  ...
  if (argc > nextarg_act) {
    if (argv[nextarg_act][0] != '.')  // "." for default
    {
      ...
      if ((argv[nextarg_act][0] == '0') && (argv[nextarg_act][1] == 'x'))
        b = 16;

      // leading "_" for base 2:
      if (argv[nextarg_act][0] == '_') {
        b = 2;
        argv[nextarg_act] += 1;
      }
...
// nextarg.cc:
void next_float_arg(double &v, const char *vname, const char *what, int argc,
                    char **argv);

void next_string_arg(char *&v, const char *vname, const char *what, int argc,
                     char **argv,
                     char *def);  // need explicit default with strings

template <typename Type>
void next_enum_arg(Type &v, const char *vname, const char *what, int argc,
                   char **argv) {
  const Type def = v;
  ++nextarg_act;
...
#define NXARG(v, what) \
  { next_arg(v, #v, what, argc, argv); }
#define NXARGFL(v, what) \
  { next_float_arg(v, #v, what, argc, argv); }
#define NXARGSTR(v, what, def) \
  { next_string_arg(v, "" #v, what, argc, argv, (char *)def); }
#define NXARGENUM(v, what) \
  { next_enum_arg(v, "" #v, what, argc, argv); }

#ifdef FXT_SILENT_NXARG
#define RESTARGS_MSG(what)  // silence
#else
#define RESTARGS_MSG(what)                                               \
  cout << "args " << nextarg_act << "," << nextarg_act + 1 << ",... : [" \
       << what << "]" << endl;
#endif

#define RESTARGS(what)  \
  {                     \
    ++nextarg_act;      \
    RESTARGS_MSG(what); \
  }
