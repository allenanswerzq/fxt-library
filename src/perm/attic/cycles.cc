...
cycles::cycles(ulong n)
    : n_(n), f_(0), nn_(n), nc_(0), nce_(), nf_(0), ma_(0), mi_(0) {
  f_ = new ulong[n_];
  ta_ = new bitarray(n);
  tt_ = new bitarray(n);
}
...
int cycles::make_cycles(const ulong *f, ulong n) {
#ifdef PARANOIA
  jjassert(n <= n_);
#endif  // PARANOIA
...
ulong cycles::next_cycle(ulong &k) const
...
int cycles::make_permutation(ulong *f, ulong n) const
...
int cycles::test_equivalent(const ulong *f, ulong n) const
...
ulong cycles::get_cycle_minima(ulong *m) const {
...
ulong cycles::get_cycle_maxima(ulong *m) const {
  ulong k = 0, len = 0, ct = 0;
  while ((len = next_cycle(k))) {
...
static void scream(const char *what) {
  cout << "+++++++++++++++++  "
       << "--- " << what << "  ---" << endl;
}
// -------------------------

void cycles::print(int info_only /*=0*/) const {
  if (0 == nce_) {
    scream("IDENTITY (only fixed points).");
    return;
  } else
    print_cycles(info_only);

  if (0 == nf_) {
    cout << "No fixed points." << endl;
  } else
    print_fixed_points(info_only);
}
// -------------------------

void cycles::print_fixed_points(int info_only /*=0*/) const {
...
void cycles::print_cycles(int info_only /*=0*/) const {
  if (!info_only) {
    ulong ct = 0;
    ulong k = 0, len = 0;
    while ((len = next_cycle(k))) {
  ...
  if (mi_ == ma_) {
    ...
    cout << " cycle lengths: " << setw(3) << mi_ << " ... " << setw(3) << ma_
         << endl;
  }
}
// -------------------------

void cycles::print_cycles_short() const {
  ulong ct = 0;
  ulong k = 0, len = 0;
  while ((len = next_cycle(k))) {
    ulong x = get(k);
...
void cycles::print_leaders() const {
  ulong k = 0, len = 0, ct = 0;
  while ((len = next_cycle(k))) {
    cout << "#" << setw(3) << ct << ":  ";
    ulong mi, ma;
...
static void print_cycle_code(const ulong *f, ulong len, std::ostream &out) {
  ulong ct = 0;
  ulong z = f[0];
  out << "  { Type t=f[" << z << "]; ";
  out << "f[" << z << "]";
...
void cycles::print_code(const char *funcname, ulong n, int use_swap /*=0*/,
                        int use_stderr /*=0*/) const {
  std::ostream &out = (use_stderr ? cerr : cout);

  cout << " ===== start printing CODE " << (use_stderr ? "(to stderr)" : "")
       << " =====" << endl;
  ...
  out << "}" << endl;
  cout << " ===== end (printing CODE) " << (use_stderr ? "to stderr" : "")
       << " =====" << endl;
