...
// Complexity O(cl).
{
  ...
  if (cl == 1)  // just remove a random position from r[]
  {
    ...
    --nr;
    swap2(r[nr], r[i]);  // remove position from set
  } else                 // cl >= 2
  {
    ...
    --cl;
    --nr;
    swap2(r[nr], r[i0]);  // remove position from set
...
template <typename Type>
inline void random_permute_cycle_type(Type *f, ulong n, const ulong *m,
                                      ulong nm, const ulong *len,
                                      ulong *tr = nullptr)
...
inline void random_cycle_type_permutation(ulong *p, ulong n, const ulong *m,
                                          ulong nm, const ulong *len,
                                          ulong *tr = nullptr)
// Create a random permutation with prescribed cycle type.
// Complexity O(n).
