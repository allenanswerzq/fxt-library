...
inline void random_permute_self_inverse(Type *f, ulong n, ulong *tr = nullptr,
                                        double *tb = nullptr, bool bi = false)
// Permute the elements of f by a random self-inverse permutation (an
// involution). Set bi:=true to signal that the branch probabilities in tb[]
//   have been precomputed (via init_involution_branch_ratios()).
{
  ulong *r = tr;
  if (tr == nullptr) r = new ulong[n];
  ...
  while (nr >= 2) {
    ...
    if (t > rat)               // 2-cycle
    {
      ...
      --nr;
      swap2(r[x2], r[nr]);  // remove from set
      swap2(f[r1], f[r2]);  // create a 2-cycle
    }
    // else fixed point, nothing to do
  }

  if (tr == nullptr) delete[] r;
  if (tb == nullptr) delete[] b;
}
// -------------------------
...
template <typename Type>
inline void random_permute_self_inverse1(Type *f, ulong n, ulong *tr = nullptr)
...
// This routine avoids the array of branch probabilities.
{
  ...
  //    // remove fixed point from set:  (not needed because cycle leader are
  //    selected randomly) ulong nfp = n - 2*n2c;  // number of fixed points
  //    while ( nfp-- )
  //    {
...
inline void random_self_inverse_permutation1(ulong *f, ulong n,
                                             ulong *tr = nullptr)
