...
inline void random_derange3(Type *f, ulong n, ulong *tr = nullptr,
...
// Must have n >= 3.
{
  ...
  while (nr >= 2) {
    ...
    } else                  // connect cycles of 3 elements and remove all
    {
      ...
      const ulong r2 = r[x2];
      --nr;
      swap2(r[x2], r[nr]);  // remove r[x2]=r2 from set:

      const ulong x3 = rand_idx(nr);  // random element !=both
      const ulong r3 = r[x3];
      --nr;
      swap2(r[x3], r[nr]);  // remove r[x3]=r3 from set:

      // random cyclic permutation of all three elements:
      swap2(f[r1], f[r3]);  // [c,b,a]
      if (rnd01() < 0.5)
        swap2(f[r1], f[r2]);  // [b,c,a]
      else
        swap2(f[r2], f[r3]);  // [c,a,b]
    }
  }
...
inline void random_derangement3(ulong *f, ulong n, ulong *tr = nullptr,
                                double *tb = nullptr, bool bi = false)
