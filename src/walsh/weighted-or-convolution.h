...
inline void slow_weighted_or_convolution(const Type *f, const Type *g,
                                         ulong ldn, Type *h, Type w)
...
template <typename Type>
inline void slow_weighted_or_convolution(const Type *f, Type *g, ulong ldn,
                                         Type w)
// Result written to g[].
{
  const ulong n = 1UL << ldn;
...
template <typename Type>
inline void weighted_or_convolution(Type *restrict f, Type *restrict g,
                                    ulong ldn, Type w)
// Compute the weighted OR-convolution h[] of f[] and g[]:
//  h[k] = sum(i | j == k,  f[i]*g[j] * (w)**bitcount(i&j))
