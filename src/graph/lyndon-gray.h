...
// Needs (2^n-2)/4 bits = (2^n-2)/32 bytes of memory.
{
 ...
 public:
  explicit lyndon_gray(ulong n, ulong ncmp = 0) : n_(n), n2_(1UL << n) {
#ifdef ALT_ALGORITM
#ifdef ALTALT
...
#define X(n) x ^= x >> n
  static inline ulong g03(ulong x) {
    X(1);
    X(2);
    return x;
  }
  static inline ulong g05(ulong x) {
    X(1);
    X(4);
    return x;
  }
  ...
  static inline ulong g29(ulong x) {
    X(1);
    X(4);
    X(8);
    X(16);
...
#undef CODE
  ...
  bool is_cycle() const { return (2 == bit_count(p_)); }
};
// -------------------------
