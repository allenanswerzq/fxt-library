...
//  arc1 points to node (2*k+1) mod ng
{
 ...
 public:
  debruijn_graph(ulong n) : ng_(2 * n) {
    a0_ = new ulong[ng_];
    a1_ = new ulong[ng_];
    init();
  }
...
// for n a power of two correspond to deBruijn sequences.
{
 ...
 public:
  debruijn_cycles(ulong n) : g_(n), ng_(g_.ng_) {
    rv_ = new ulong[ng_];
    qq_ = new ulong[ng_];
  ...
  void print_path() const { ::print_path(rv_, ng_); }

  void print_bin_path() const { ::print_bin_path(rv_, ng_, ngbits_); }
