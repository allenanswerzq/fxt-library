...
ulong digraph::num_edges(ulong p) const
...
ulong digraph::edge_idx(ulong p, ulong pn) const
...
static inline int cmp0(const ulong &a, const ulong &b) { return -cmp1(a, b); }
// -------------------------

void digraph::sort_edges(int rq /*=1*/) {
  if (rq)
    sort_edges(cmp0);
  else
    sort_edges(cmp1);
...
void digraph::sort_edges(int (*cmp)(const ulong &, const ulong &)) {
  if (nullptr == vn_)  // value == index (in e[])
  {
    for (ulong k = 0; k < ng_; ++k) {
      ulong x = ep_[k];
      ulong n = ep_[k + 1] - x;
...
ulong digraph::test_edge_sorted(int (*cmp)(const ulong &, const ulong &)) const
...
bool digraph::is_edge_sorted(int (*cmp)(const ulong &, const ulong &)) const {
  return (ng_ == test_edge_sorted(cmp));
}
// -------------------------

void digraph::reverse_edge_order(ulong p0, ulong p1 /*=0*/)
...
void digraph::randomize_edge_order(ulong p0, ulong p1 /*=0*/)
...
void digraph::print(const char *bla /*=0*/) const
...
void digraph::print_horiz(const char *bla /*=0*/) const
// Print directed graph.
{
  if (bla) cout << bla << endl;
  cout << setw(7) << "Node:";
...
ulong digraph::test() const {
  ulong ng = ng_;
  for (ulong k = 0; k < ng; ++k)
    if (ep_[k] > ep_[k + 1]) return 1;

  ulong ne = num_edges();
  for (ulong k = 0; k < ng; ++k)
    if (ep_[k] >= ne) return 2;

  if (ep_[ng] != ne) return 3;

  for (ulong k = 0; k < ne; ++k)
    if (e_[k] >= ng_) return 10;
