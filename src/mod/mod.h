...
// Copyright (C) 2010, 2012, 2013, 2014, 2017, 2018 Joerg Arndt
...
// Modular type and arithmetic operations on it
{
 ...
 public:
  ...
  static mod *cos,
      *isin;  // [max2pow+1];   // cos/i*sin corresponding to root_2pow
  static mod *cosm,
      *isinm;  // [max2pow+1];  // cos/sin corresponding to root_m2pow

 public:
  explicit mod() : x_(0) { ; }

  explicit mod(const umod_t i) : x_((i < modulus ? i : i % modulus)) { ; }

  mod(const mod &m) : x_(m.x_) { ; }
  ~mod() { ; }

  static bool init(umod_t m, factorization *mf = nullptr);
  friend bool mod_initialize(umod_t m, umod_t *primes /*=nullptr*/);
  ...
  umod_t order() const { return order_mod(x_, mod::modulus, mod::phifact); }
  ...
  inline mod &operator=(umod_t i) {
    (*this) = mod(i);
    return *this;
  }
  inline mod &operator=(uint i) {
    (*this) = mod(i);
    return *this;
  }
  inline mod &operator=(ulong i) {
    (*this) = mod(i);
    return *this;
  }
  ...
  friend inline mod &operator++(mod &z) {
    ++z.x_;
    if (z.x_ == mod::modulus) z.x_ = 0;
    return z;
  }

  friend inline mod &operator--(mod &z) {
    if (z.x_ == 0) z.x_ = mod::modulus;
    z.x_--;
    return z;
  }

  friend inline mod &operator+=(mod &z, const mod &h) {
    z.x_ = add_mod(z.x_, h.x_, mod::modulus);
    return z;
  }

  friend inline mod &operator-=(mod &z, const mod &h) {
    z.x_ = sub_mod(z.x_, h.x_, mod::modulus);
    return z;
  }

  friend inline mod &operator*=(mod &z, const mod &h) {
    z.x_ = mul_mod(z.x_, h.x_, mod::modulus);
    return z;
  }
