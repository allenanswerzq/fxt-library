...
bool mod::init(umod_t m, factorization *mf /*=0*/) {
  bool q;
  if (mf)
    q = mod_initialize(m, (*mf).prime_);
  else
    q = mod_initialize(m, nullptr);
  return q;
}
// -------------------------
...
mod mod::root2pow(int ldorder)
// Return root of order 2**ldorder
{
  if (Abs(ldorder) > (long)max2pow) return mod::zero;
  if (ldorder >= 0)
    return root_2pow[ldorder];
  else
    return root_m2pow[-ldorder];
}
// -------------------------

mod mod::cos2pow(int ldorder)
