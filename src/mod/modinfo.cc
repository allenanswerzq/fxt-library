...
void mod_info0() {
  cout << "-------- start MOD_INIT():  m=" << mod::modulus << " --------"
       << endl;
}
// -------------------------

void mod_info99() { cout << "-------- end MOD_INIT(). --------" << endl; }
// -------------------------

void mod_info1() {
  umod_t m = mod::modulus;

  cout << "modulus= " << m << " == 0x" << hex << m << dec << endl;
...
static inline uint highest_one_idxp1(umod_t m) {
  uint k = 0;
  while (m) {
    k++;
    m >>= 1;
...
void mod_info2() {
  cout << "euler_phi(modulus)= " << (mod::phi) << " == 0x" << hex << (mod::phi)
       << dec << " == " << mod::phifact << endl;

  cout << "maxorder= " << (mod::maxorder) << " == 0x" << hex << (mod::maxorder)
       << dec << endl;
}
// -------------------------
...
void mod_info3() {
  ...
  cout << "maxordelem= " << (mod::maxordelem) << " == 0x" << hex
       << (mod::maxordelem) << dec << endl;
...
void mod_info4() {
  ...
  cout << "max2pow= "
       ...
       << "   (max FFT length = 2**" << m2 << " == " << ((umod_t)1 << m2) << ")"
       << endl;
  ...
  cout << "root2pow(max2pow)=" << mod::root2pow(m2)
       << "   root2pow(-max2pow)=" << mod::root2pow(-m2) << endl;
  ...
  if (m2 > 0) {
    if ((mod::root2pow(1)).x_ != (mod::modulus - 1ULL))
      cout << "root2pow(1)=" << mod::root2pow(1) << " != -1  (!)" << endl;
  }
