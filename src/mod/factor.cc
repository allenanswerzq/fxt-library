...
factorization::factorization() { reset(); }
// -------------------------

factorization::factorization(umod_t n, const umod_t *f /*=nullptr*/)
//
...
void factorization::sort_by_primes()
// sort wrt. primes
{
...
ulong factorization::exponent_of(umod_t p) const
// return exponent of prime p in factorization
{
  for (ulong i = 0; i < npr_; ++i)
    if (p == prime_[i]) return expon_[i];
  return 0;
}
// -------------------------

void factorization::print(const char *bla, std::ostream &os) const {
  if (bla) os << bla;
...
//
{
  ...
  if (q == 0)
    ...
    do {
      n = q;
      ++ex;
      //            cout << " :: divide_out_factor(): n=" << n << "  ex=" << ex
      //            << endl;
      q = get_factor_q(n, v);
    } while (q != 0);
    return ex;
