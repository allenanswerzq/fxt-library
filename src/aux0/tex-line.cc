...
// vertical line segments, else the HORIZONTAL coordinate.
{
  ...
  if (vq)
    ...
    cout << "{\\line(";
  cout << sx << "," << sy << ")"
       << "{" << setprecision(3) << vlen << "}}" << endl;

#else  // TEX_USE_PICT2E

  cout << "\\curve(" << x0 << "," << y0 << ", " << x1 << "," << y1 << ")"
       << endl;
...
static inline double chop0(double x) {
  if (fabs(x) < 0.001) x = 0.0;
  return x;
}
...
void tex_draw_turn(double x, double y, double dx, double dy, double ndx,
                   double ndy, ulong style, bool vq /*=true*/,
                   double m /*=0.25*/)
...
// Set vq to draw lines as vectors.
{
  ...
  double x3 = nx + m * ndx,
         y3 = ny + m * ndy;  // X3 = NX + m * NDX = X + DX + m * NDX
