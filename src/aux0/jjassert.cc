...
void set_fail_action(int a /*=JJ_ASSERT_DEFAULT*/) { fail_action = a; }
// -------------------------

void jjassert_fail(const char *func, const char *pretty_func, const char *file,
                   const int line, const char *expr) {
  //    setvbuf(stdout, NULL, _IONBF, 0); // changes stdout to unbuffered
  //    (_IONBF) mode.
...
do_fail:
  switch (fail_action) {
    case JJ_ASSERT_IGNORE:;
      break;  // happily continue into disaster

    case JJ_ASSERT_ABORT:
      abort();  // break;  // no return
      // See man 2 abort
