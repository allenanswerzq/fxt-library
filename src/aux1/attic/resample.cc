...
ulong resample_sinc(const double *a, ulong an, double af, double *b, double bf,
                    ulong bn)
...
// Must have: v>0
{
  ...
  for (ulong j = 0; j < n; j++) {
    ...
    b[j] = (a[i] * sinc(2 + r) + a[i + 1] * sinc(1 + r) + a[i + 2] * sinc(r) +
            a[i + 3] * sinc(1 - r) + a[i + 4] * sinc(2 - r) +
            a[i + 5] * sinc(3 - r));  //*0.5;
  }
...
ulong sample_down_linear(const double *f, ulong nf, double v, double *g,
                         ulong ng)
//
// Uses linear interpolation (if noninteger v).
// Return number of values produced.
...
{
  if (v >= 1.0)
    return sample_down_linear(f, nf, v, g, ng);
  else
    return sample_up_linear(f, nf, v, g, ng);
}
// -------------------------
