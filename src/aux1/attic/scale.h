...
void linear_scale(Type *f, ulong n, Type mi, Type ma,  // old min, max
...
template <typename Type>
void linear_scale(Type *f, ulong n, Type nmi, Type nma)  // new min, max
// linear scale so that
//   new_min==nmi,  ne_max==nma
{
  Type mi, ma;
...
template <typename Type>
void log_scale(Type *f, ulong n, double xi, double xa, Type nma) {
  Type mi, ma;
  min_max(f, n, &mi, &ma);
