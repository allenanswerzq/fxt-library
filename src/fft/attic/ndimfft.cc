...
void fft_4dim(double *fr, double *fi, ulong d1, ulong d2, ulong d3, ulong d4,
              int is);
void fft_5dim(double *fr, double *fi, ulong d1, ulong d2, ulong d3, ulong d4,
              ulong d5, int is);
...
// Compute 4-dimensional FFT.
{
  ...
  for (ulong k = 0; k < d123; k++)
    skip_fft(fr + k, fi + k, d4, d123, wr, wi, is);
}
// -------------------------
...
// Compute 5-dimensional FFT.
{
  ...
  for (ulong k = 0; k < n; k += d1234)
    fft_4dim(fr + k, fi + k, d1, d2, d3, d4, is);

  for (ulong k = 0; k < d1234; k++)
    skip_fft(fr + k, fi + k, d5, d1234, wr, wi, is);
}
// -------------------------
