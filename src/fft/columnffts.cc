...
void column_ffts(Complex *f, ulong nr, ulong nc, int is, int zp, Complex *tmp)
//
// nr x nc matrix (nr rows, nc columns)
// length of each col is nr
// length of each row is nc
...
void column_real_complex_ffts(double *f, ulong nr, ulong nc, int zp,
                              double *tmp)
//
// nr x nc matrix (nr rows, nc columns)
// length of each col is nr
...
//
{
  ...
  for (ulong k = 0; k < nc; ++k) {
    if (zp)
      skip_real_complex_fft0(f, nr, nc, tr);
    else
      skip_real_complex_fft(f, nr, nc, tr);
    f++;
  }
...
void column_complex_real_ffts(double *f, ulong nr, ulong nc, double *tmp)
//
// nr x nc matrix (nr rows, nc columns)
// length of each col is nr
// length of each row is nc
...
void column_complex_imag_ffts(const double *fr, double *fi, ulong nr, ulong nc,
                              double *tmp)
//
// nr x nc matrix (nr rows, nc columns)
// length of each col is nr
