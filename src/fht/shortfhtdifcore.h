...
{  // kh=2
  ...
  {  // ---- i=1
    double c1 =
        .923879532511286756128183189397;  // == cos(Pi*1/8) == cos(Pi*1/8)
    double s1 =
        .382683432365089771728459984029;  // == sin(Pi*1/8) == sin(Pi*1/8)
    // c2 = s2 = sqrt(1/2)
    // do loop II:
...
{  // kh=4
  ...
  {  // ---- i=1
    double c1 =
        .980785280403230449126182236134;  // == cos(Pi*1/16) == cos(Pi*1/16)
    double s1 =
        .195090322016128267848284868476;  // == sin(Pi*1/16) == sin(Pi*1/16)
    double c2 =
        .923879532511286756128183189397;  // == cos(Pi*2/16) == cos(Pi*1/8)
    double s2 =
        .382683432365089771728459984029;  // == sin(Pi*2/16) == sin(Pi*1/8)
    // do loop II:
  ...
  {  // ---- i=3
    double c1 =
        .831469612302545237078788377618;  // == cos(Pi*3/16) == cos(Pi*3/16)
    double s1 =
        .555570233019602224742830813947;  // == sin(Pi*3/16) == sin(Pi*3/16)
    double c2 =
        .382683432365089771728459984032;  // == cos(Pi*6/16) == cos(Pi*3/8)
    double s2 =
        .923879532511286756128183189396;  // == sin(Pi*6/16) == sin(Pi*3/8)
    // do loop II:
...
{  // kh=8
  ...
  {  // ---- i=1
    double c1 =
        .995184726672196886244836953109;  // == cos(Pi*1/32) == cos(Pi*1/32)
    double s1 =
        .098017140329560601994195563888;  // == sin(Pi*1/32) == sin(Pi*1/32)
    double c2 =
        .980785280403230449126182236134;  // == cos(Pi*2/32) == cos(Pi*1/16)
    double s2 =
        .195090322016128267848284868476;  // == sin(Pi*2/32) == sin(Pi*1/16)
    // do loop II:
  ...
  {  // ---- i=2
    ...
    double s1 =
        .195090322016128267848284868476;  // == sin(Pi*2/32) == sin(Pi*1/16)
    double c2 =
        .923879532511286756128183189397;  // == cos(Pi*4/32) == cos(Pi*1/8)
    double s2 =
        .382683432365089771728459984029;  // == sin(Pi*4/32) == sin(Pi*1/8)
    // do loop II:
    cmult(s2, c2, f[18], f[30], b, a);
    sumdiff(f[16], a, f0, f1);
    sumdiff(f[28], b, g0, g1);
  ...
  {  // ---- i=3
    double c1 =
        .956940335732208864935797886980;  // == cos(Pi*3/32) == cos(Pi*3/32)
    double s1 =
        .290284677254462367636192375816;  // == sin(Pi*3/32) == sin(Pi*3/32)
    double c2 =
        .831469612302545237078788377618;  // == cos(Pi*6/32) == cos(Pi*3/16)
    double s2 =
        .555570233019602224742830813947;  // == sin(Pi*6/32) == sin(Pi*3/16)
    // do loop II:
  ...
  {  // ---- i=5
    double c1 =
        .881921264348355029712756863661;  // == cos(Pi*5/32) == cos(Pi*5/32)
    double s1 =
        .471396736825997648556387625904;  // == sin(Pi*5/32) == sin(Pi*5/32)
    double c2 =
        .555570233019602224742830813950;  // == cos(Pi*10/32) == cos(Pi*5/16)
    double s2 =
        .831469612302545237078788377616;  // == sin(Pi*10/32) == sin(Pi*5/16)
    // do loop II:
  ...
  {  // ---- i=6
    ...
    double s1 =
        .555570233019602224742830813947;  // == sin(Pi*6/32) == sin(Pi*3/16)
    double c2 =
        .382683432365089771728459984032;  // == cos(Pi*12/32) == cos(Pi*3/8)
    double s2 =
        .923879532511286756128183189396;  // == sin(Pi*12/32) == sin(Pi*3/8)
    // do loop II:
    cmult(s2, c2, f[26], f[22], b, a);
    sumdiff(f[24], a, f0, f1);
    sumdiff(f[20], b, g0, g1);
  ...
  {  // ---- i=7
    double c1 =
        .773010453362736960810906609759;  // == cos(Pi*7/32) == cos(Pi*7/32)
    double s1 =
        .634393284163645498215171613224;  // == sin(Pi*7/32) == sin(Pi*7/32)
    double c2 =
        .195090322016128267848284868478;  // == cos(Pi*14/32) == cos(Pi*7/16)
    double s2 =
        .980785280403230449126182236133;  // == sin(Pi*14/32) == sin(Pi*7/16)
    // do loop II:
