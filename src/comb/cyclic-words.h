...
// In other words:
// among all rotations of W, the one shifted by i letters (to the left) is
// lex-min.
//.
...
// Compare A[ai, ai+1, ...] and B[bi, bi+1, ...] cyclically (indices taken mod
// n). Return  +1 if A[] > B[],  -1 if A[] < B[],  otherwise (A[]==B[]) 0.
{
  for (ulong j = 0; j < n; ++j) {
...
// Compare A[ai, ai+1, ...] and B[bi, bi+1, ...] cyclically (indices taken mod
// n). Return  +1 if A[] > B[],  -1 if A[] < B[],  otherwise (A[]==B[]) return
// 0.
{
#if 1
