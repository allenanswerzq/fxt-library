...
{
 public:
  explicit ruler_func_s(ulong n, ulong s) : composition_nz_sorts(n + 1, s - 1) {
    first();
  }

  ~ruler_func_s() { ; }

  void first() { composition_nz_sorts::first(); }

  ulong data() const { return composition_nz_sorts::last_part(); }
