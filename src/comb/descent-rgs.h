...
// See OEIS sequence A225588.
{
 ...
 public:
  ...
  void first() {
    ...
    //        for (ulong k=0; k<n_; ++k)  m_[k] = k;  // for non-changes:
    //        A000110
  }
  ...
  // Return position of leftmost change, zero with last.
  {
    ...
    //        const ulong na = m_[j-1] + ( a_[j] > a_[j-1] );  // ascents:
    //        A022493 for (ulong z=j; z<n_; ++z)  m_[z] = na;
    ...
    //        ulong na = m_[j-1] + ( a_[j] == a_[j-1] );  // non-changes:
    //        A000110 m_[j] = na; for (ulong z=j+1; z<n_; ++z) {
    //        na+=(a_[z-1]==a_[z]);  m_[z] = na; }
