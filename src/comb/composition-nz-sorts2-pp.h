...
// A052934 (s=5), A055272 (s=6), A055274 (s=7), and A055275 (s=8).
{
 public:
  ...
  composition_nz_sorts2_pp &operator=(const composition_nz_sorts2_pp &) =
      delete;

 public:
  explicit composition_nz_sorts2_pp(ulong n, ulong ns,
                                    const ulong *vns = nullptr)
  // Should have: ns >= 1.
  {
    n_ = n;
#ifndef COMPOSITION_NZ_SORTS2_PP_FIXARRAYS
  ...
  {
    if (aq)
      return is_sorts_in_runs_sorted_asc(data(), sdata(), m_);
    else
      return is_sorts_in_runs_sorted_desc(data(), sdata(), m_);
  }
