...
bool lindenmayer_system::set_all_maps(const std::vector<std::string>& maps) {
  ulong nr = maps.size();
  if ((nr & 1) == 1) {
...
std::vector<std::string> lindenmayer_system::str_all_maps(ulong z /*=0*/) const
...
void lindenmayer_system::print_all_maps(ulong z /*=0*/) const
