...
//   and A225618 (weak inversions).
{
 ...
 private:
  ...
  // At return st[1,2,...,m-1] is valid for a[0,1,...,j-1].
  {
    ...
    for (ulong d1 = d + 1; d1 <= mm;
         ++d1)  // optimized, scanning fewer candidates
    {
 ...
 public:
  ...
  // Always true for s==0.
  {
    ...
    for (ulong j = md; j > 0; --j)
      if (st_[j - 1] - st_[j] < s) return false;
    return true;
#endif
  }
  ...
  void print1(const char *bla) const { print_vec1(bla, data(), n_); }
