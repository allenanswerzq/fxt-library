...
// Multiset permutations in lexicographic order, iterative algorithm.
{
 protected:
  ...
  ulong
      *r_;  // number of elements '0' in r[0], '1' in r[1], ..., 'k-1' in r[k-1]
  ulong n_;  // number of objects
  ulong
      *ms_;  // multiset data in ms[0], ..., ms[n-1], sentinels at [-1] and [-2]
 ...
 public:
  ...
  // return n with last permutation.
  {
    ...
    do {
      --i;
    } while (ms_[i] >= ms_[i + 1]);  // can read sentinel
