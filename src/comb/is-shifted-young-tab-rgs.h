...
inline bool is_delayed_young_tab_rgs(const ulong *A, ulong n, ulong h,
                                     ulong s = 1)
...
// Always true for s==0.
{
  ...
  for (ulong j = md; j > 0; --j)
    if (P[j - 1] - P[j] < s) return false;

  return true;
}
// -------------------------

inline bool is_shifted_young_tab_rgs(const ulong *A, const ulong *P, ulong n,
                                     ulong h, ulong s = 1)
