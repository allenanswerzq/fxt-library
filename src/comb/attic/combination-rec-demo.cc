...
void next_rec(ulong d) {
  ...
  if ((rv1 == n - 1 - r) || (0 == r)) {
    ...
    switch (rq) {
      ...
      case 2:
        q = rv1 & 1;
        break;  // near-perfect order
      case 3:
        q = (d ^ rv1) & 1;
        break;  // diagonal gray order
      default:
        q = rand_bit();  // "random" order
    }
...
//  for (( n=0; n<=7; ++n )); do echo "$n:  "; for (( k=0; k<=n; ++k )); do
//  ./bin $n $k 0 | grep rct | sed -r 's/rct=/ /;' ; done; done
//
//  0:   0
//  1:   0 0
//  2:   0 1 0
