...
#define PERM_LEX_OPT  // to use easy case optimization
...
#ifdef PERM_LEX_OPT
  ulong ct_;  // counter to detect easy case
#endif
...
#endif
    {  // Based on code by Glenn Rhoads (who attributes the algorithm to
       // Dijkstra):
      ...
      do {
        --i;
      } while (p_[i] > p_[i + 1]);    // can read sentinel
