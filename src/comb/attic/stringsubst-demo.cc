...
// Cf. comb/stringsubst-hilbert3d-demo.cc for the 3D Hilbert curve via string
// substitution.

static void usage_err(const char *argv0) {
  cerr << "Usage:" << endl;
  cerr << "  " << argv0 << " maxdepth starts symbol0 rule0 symbol1 rule1 ... "
       << endl;
...
bool run_substitutions(ulong maxn, const char *start, ulong nsym,
                       const char **args) {
#ifdef MAKE_UTIL
...
int main(int argc, char *argv[]) {
  ...
  else  // defaults:
  {
    ...
    const char *rabbit_rules[] = {"0", "1", "1", "10"};
    ret = run_substitutions(7, "0", 2, rabbit_rules);
    //        ret = run_substitutions( 7, "0", 2, { "0", "1", "1", "10" } );  //
    //        needs -std=c++0x

    cout << "\n------------- Thue-Morse sequence: -------------" << endl;
    const char *thue_morse_rules[] = {"0", "01", "1", "10"};
    ret = run_substitutions(6, "0", 2, thue_morse_rules);

    cout << "\n------------- Dragon curve (90deg turns): -------------" << endl;
    const char *dragon_rules[] = {"F", "F+G+", "G", "-F-G", "+", "+", "-", "-"};
