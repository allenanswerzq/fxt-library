...
bool partition_gen::check(ulong i /*=0*/) const {
  ulong xx = 0;
...
void partition_gen::print() const
// Print in the form (6 ==)
//   1 + 1 + 4
{
  //    cout << setw(4) << x_ << " == ";
...
void partition_gen::print2() const
// Print in the form (6 ==)
//  2* 1 + 0    + 0    + 1* 4 + 0    + 0
{
...
ulong partition_gen::next_func(ulong i) {
start:
  if (0 != i) {
...
ulong partition_gen::count(ulong x)
...
ulong partition_gen::count_func(ulong i) {
  if (0 != i) {
    while (r_[i] > 0) {
      pc_[i - 1] = 0;
      r_[i - 1] = r_[i];
