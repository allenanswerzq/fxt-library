...
int setpart::cp_append(const int *src, int *dst, int k, int a)
...
void setpart::first(bool xdr, int dr0) {
  dr0 = ((dr0 >= 0) ? +1 : -1);
  dr0_ = dr0;
...
void setpart::print_internal() const {
  //    const char sep[]=", ";
  const char sep[] = " ";
...
int setpart::print_p(int k) const {
  const int *x = pp_[k];
  int n = k;
