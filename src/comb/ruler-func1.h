...
{
 public:
  explicit ruler_func1(ulong n) : composition_nz(n + 1) { first(); }

  ~ruler_func1() { ; }

  void first() { composition_nz::first(); }

  ulong data() const { return composition_nz::last_part(); }
  ...
  void print_composition(const char *bla) const { composition_nz::print(bla); }
};
// -------------------------
