...
//  A025160 (d=6), A025161 (d=7), and A025162 (d=8).
{
 public:
  ...
  partition_dist_d_asc(const partition_dist_d_asc &) = delete;
  partition_dist_d_asc &operator=(const partition_dist_d_asc &) = delete;
 ...
 public:
  ...
  ~partition_dist_d_asc() { delete[] a_; }
  ...
  bool OK() const {
    ...
    for (ulong j = 1; j < m_; ++j)  // min difference d?
      if (a_[j + 1] - a_[j] < d_) return false;
