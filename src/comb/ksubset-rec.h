...
// Recursive CAT algorithm.
{
 public:
  ...
  ksubset_rec(const ksubset_rec &) = delete;
  ksubset_rec &operator=(const ksubset_rec &) = delete;
 ...
 public:
  ...
  void generate(void (*visit)(const ksubset_rec &, ulong), ulong kmin,
                ulong kmax, ulong rq, ulong nq = 0) {
    ct_ = 0;
    rct_ = 0;
