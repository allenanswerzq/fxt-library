...
//   http://arxiv.org/abs/1405.6503
{
 protected:
  ...
  ulong tr_;  // aux: current track, index of last non-zero digit, 0 also for
              // all-zero word
  ulong
      *a_;  // digits of mixed radix number (multiplicity of kind k in subset).
  ulong *m1_;  // nines (radix minus one) for each digit (multiplicity of kind k
               // in superset).
 ...
 public:
  ...
  ulong rank() const { return RU->rank(); }

  void unrank(ulong r) { tr_ = RU->unrank(r, a_); }
