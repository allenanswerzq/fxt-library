...
// Original documentation at end of file.
{
 ...
 public:
  ...
  void get_swaps(ulong &sw1, ulong &sw2) const {
    sw1 = sw1_;
    sw2 = sw2_;
  }
  ...
  bool next() {
    ...
    sw1_ = i - 1;
    sw2_ = l - 1;  // save positions swapped
    swap2(ms_[i], ms_[l]);

    D_[l] = e;
    D_[i] = d;  // restore drifts of head and neighbor
    P_[l] = p;
    Q_[p] = l;  // wrap neighbor around to other end
