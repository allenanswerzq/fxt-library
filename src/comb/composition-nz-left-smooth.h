...
// Same as "fountains of coins", see OEIS sequence A005169.
{
 public:
  ...
  composition_nz_left_smooth(const composition_nz_left_smooth &) = delete;
  composition_nz_left_smooth &operator=(const composition_nz_left_smooth &) =
      delete;
 ...
 public:
  ...
  // Return 0 if current composition is last.
  {
    ...
    while (a_[j] > a_[j - 1])
    //        while ( a_[j] > a_[j-1] - 1 )  // partitions in weakly descending
    //        order
    {
      z += a_[j];
      a_[j] = 1;
