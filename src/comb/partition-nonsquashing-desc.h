...
// decreasing).
{
 public:
  ...
  partition_nonsquashing_desc &operator=(const partition_nonsquashing_desc &) =
      delete;
 ...
 private:
  ...
  // Undefined for s == 0.
  {
    do {
      ulong h;
      if (sd_)
        h = s / 2 + 1;  // strongly decreasing: A040039
      else
        h = (s + 1) / 2;  // non-squashing: A018819
      a_[j] = h;
      ++j;
      s -= h;
    } while (s);
 ...
 public:
  ...
  ~partition_nonsquashing_desc() { delete[] a_; }
  ...
  bool OK() const {
    ...
    for (ulong j = m_; j != 0; --j)  // non-squashing ?
    {
      ...
      //            if ( w>=v )  return false; // strongly decreasing, cf.
      //            A040039
      w += v;
    }
#endif
