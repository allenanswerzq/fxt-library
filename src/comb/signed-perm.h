...
// There are n! * 2^n signed reflections of n elements.
{
 ...
 public:
  signed_perm(ulong tn) : P(tn), R(tn), n(tn) {
    refl_q = new bool[n];
    first();
  }
  ~signed_perm() { delete[] refl_q; }
 ...
 public:
  ...
  bool is_rotation() const { return refl_is_rot ^ perm_is_rot; }

  const ulong* perm_data() const { return P.data(); }
  ...
  void print(const char* bla, bool offset_one_q = false) const {
    ...
    cout << "(";
    for (ulong j = 0; j < n; ++j) {
      cout << (refl_data()[j] ? '-' : '+') << perm_data()[j] + offset_one_q
           << (j + 1 == n ? "" : ", ");
    }
    cout << ")";
