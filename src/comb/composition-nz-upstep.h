...
// Max up-step 0 gives partitions as weakly descending lists.
{
 public:
  ...
  composition_nz_upstep(const composition_nz_upstep &) = delete;
  composition_nz_upstep &operator=(const composition_nz_upstep &) = delete;
 ...
 public:
  ...
  ~composition_nz_upstep() { delete[] a_; }
  ...
  bool OK() const {
    ...
    for (ulong j = 2; j <= m_; ++j)  // check that up-steps are <= u
      if (a_[j] > a_[j - 1] + u_) return false;
