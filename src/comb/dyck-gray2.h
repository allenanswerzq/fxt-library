...
//   Journal of Discrete Algorithms, vol.4, no.4, pp.633-648, (December-2006)
{
 ...
 public:
  ...
  // must have tk>=2, tm>=1
  {
    ...
    c_ = new ulong[m + 2];
    // sentinels c_[0] (with computing MN) and c_[m+1] (with condition in
    // next())
  ...
  void first() {
    ...
    for (ulong j = 0; j <= m; ++j)
      s_[j] = 0;  //  {s_[j] = 0 for 1 <= j <= m}  // (all at first value)
    for (ulong j = 0; j <= m; ++j)
      p_[j] = false;  //  {p_[j] = 0 for 1 <= j <= m}  // actually no need to
                      //  initialize
    for (ulong j = 0; j <= m; ++j)
      c_[j] = j;    // first word == [1, 2, 3, ..., m]
  ...
  // Comments in curly braces {like this} are from the paper.
  {
    ...
    const ulong MN =
        c_[i - 1] +
        1;  // {MN is the minimum value of c_[i]}  // can read sentinel c_[0]
    ...
    if (s_[i] == 0)  // { c_[i] is at its first value }
    {
      ...
      //            if ( (k==2) && (i<m) && (c_[i+1]==MX+2) )  // {see above
      //            comment} if ( (k==2) && (c_[i+1]==MX+2) )  // can read
      //            sentinel c_[m+1]
      if (c_[i + 1] ==
          MX + k)  // can read sentinel c_[m+1]==0  // jj-NOTE: is this correct?
      {
        p_[i] = 1 - p_[i];
      }
    }
    ...
    {
      s_[i] =
          0;  // {c_[i] will be at its first value the next time i is the pivot}
      e_[i] = e_[i - 1];
      e_[i - 1] = i - 1;
    }
