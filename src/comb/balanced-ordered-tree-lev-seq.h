...
// See OEIS sequences A079500 and A007059.
{
 ...
 private:
  ...
  balanced_ordered_tree_lev_seq(const balanced_ordered_tree_lev_seq &) = delete;
  balanced_ordered_tree_lev_seq &operator=(
      const balanced_ordered_tree_lev_seq &) = delete;
 ...
 public:
  ...
  ~balanced_ordered_tree_lev_seq() { delete[] a_; }
 ...
 public:
  ...
  ulong num_leaves() const { return aux.num_leaves(data(), size()); }
  ...
  ulong min_limb_length() const { return aux.min_limb_length(data(), size()); }

  ulong max_limb_length() const { return aux.max_limb_length(data(), size()); }

  ulong height() const { return aux.height(data(), size()); }

  void print(const char *bla, bool dfz = false) const {
    print_vec(bla, data(), size(), dfz);
  }

  void print_aa(const char *bla) const { aux.print_aa(bla, data(), size()); }
 ...
 private:
  bool is_balanced() const { return aux.is_balanced(data(), size()); }
