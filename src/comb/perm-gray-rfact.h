 ...
 public:
  ...
  bool next() {
    ...
    if (j <= 1)  // easy cases: swap == (0,j+1)
    {
      ...
      if (d >
          0)  // in the inverse permutation search first smaller element left:
      {
        for (x2 = x1 - 1;; --x2)
          if ((i2 = ix_[x2]) < i1) break;
        // jjassert( x2 == x1-1 );  // not true: first failure for transition
        // 59/60 jjassert( (long)x2 >= (long)x1-2 );  // not true: 197/198
      } else  // in the inverse permutation search first smaller element right:
      {
        for (x2 = x1 + 1;; ++x2)
          if ((i2 = ix_[x2]) < i1) break;
        // jjassert( x2 == x1+1 );  // not true: first failure for transition
        // 41/42 jjassert( (long)x2 <= (long)x1+2 );  // not true: 281/282
      }
