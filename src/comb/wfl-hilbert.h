...
// Container used in class wfl_hilbert.
{
 public:
  ...
  //    ulong cvertex; // vertex within super-cell,  // jj: this was a
  //    write-only variable
  ulong
      cedge;  // transforming edge-pair or corresponding symbol indexing tables
...
// for Fred Lunnon's explanations of the algorithm.
{
 public:
  ...
  wfl_cell
      branch[WFL_NUMLEV];  // chain along branch of tree of cells / D0L symbols
 ...
 public:
  ...
  ulong gray(ulong j) const { return j ^ (j >> 1); }
  ...
  // Convert path pace to unit hypercube edge-pair decorating path
  {
    ...
    if ((j & 1) == 0)
      return (gray(j - 2) << dim) | gray(j + 1);
    else
      return (gray(j - 1) << dim) | gray(j + 2);
  }
