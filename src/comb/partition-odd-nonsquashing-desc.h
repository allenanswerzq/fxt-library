...
// Cf. OEIS sequence A187821.
{
 public:
  ...
  partition_odd_nonsquashing_desc(const partition_odd_nonsquashing_desc &) =
      delete;
  partition_odd_nonsquashing_desc &operator=(
      const partition_odd_nonsquashing_desc &) = delete;
 ...
 private:
  ...
  // Undefined for s == 0.
  {
    do {
      ...
      //            ulong h = ??;  // strongly decreasing, odd parts; need to
      //            edit next() as well (?) ulong h = (s+1) / 2;  //
      //            non-squashing (all parts) ulong h = s / 2 + 1;  // strongly
      //            decreasing (all parts)
      a_[j] = h;
      ++j;
      s -= h;
    } while (s);
 ...
 public:
  ...
  ~partition_odd_nonsquashing_desc() { delete[] a_; }
