...
// Loopless algorithm.
{
 public:
  ...
  partition_odd_asc_subset_lex_csh(const partition_odd_asc_subset_lex_csh &) =
      delete;
  partition_odd_asc_subset_lex_csh &operator=(
      const partition_odd_asc_subset_lex_csh &) = delete;
  ...
  // Loopless algorithm.
  {
    ...
    } else  // add to the left
    {
      ...
      } else  // add to second or third left
      {
        ...
        if (x2 <=
            z2)  // add 2 units to third left, repeat part, put rest at end
