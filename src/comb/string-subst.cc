...
bool string_subst::add_map(uchar c, const uchar *const str) {
  if (c == '\0') {
    cerr << err_msg_head << " add_map(): char == 0 forbidden." << endl;
    return false;
  }

  if (map_start_[(ulong)c] != NULL) {
    cerr << err_msg_head << " add_map(): map for "
  ...
  do  // can only fail with invalid call from within class
  {
    ...
    if (end_map_ptr_ >= maps_ + maxrlen_) {
      cerr << err_msg_head << " add_map(): internal error:"
...
bool string_subst::add_map(const uchar *const c, const uchar *const str) {
  if (c[0] == '\0') {
    cerr << err_msg_head << " add_map(): char == 0 forbidden." << endl;
    return false;
  }
  ...
  if (c[1] != '\0') {
    cerr << err_msg_head << " add_map(): left side more than one symbol."
         << endl;
    return false;
  }
...
bool string_subst::set_axiom(const char *const ax, bool change_q /*=false*/) {
...
bool string_subst::set_all_maps(const char *const maps[], ulong nr) {
  ...
  for (ulong j = 1; j < nr; j += j) rsz += strlen(maps[j]) + 1;
...
void string_subst::print_maps() const {
...
void string_subst::print_internal_state() const {
  print_maps();
...
ulong string_subst::num_maps() const {
  ulong ct = 0;
