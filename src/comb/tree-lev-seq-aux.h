...
// Auxiliary class for classes *tree_lev_seq*
{
 ...
 public:
  ...
  ulong num_leaves(const ulong *a, ulong n) const {
    ...
    for (ulong j = 1; j < n; ++j) r += (a[j - 1] + 1 != a[j]);
    return r;
  }
  ...
  // Out-degree of node[j].
  {
    ...
    ulong r =
        0;  // out-degree: number of heights t+1 before next occurrence of t
 ...
 public:
  ...
  void print_branching_numbers(const char *bla, const ulong *a, ulong n,
                               bool dfz) const;

  void print_base_seq(const char *bla, const ulong *a, ulong n,
                      bool dfz = true) const;

  void print_paren_word(const char *bla, const ulong *a, ulong n,
                        const char *s = "()") const;

  // only for balanced trees:
  void print_balanced_composition(const char *bla, const ulong *a,
                                  ulong n) const;
