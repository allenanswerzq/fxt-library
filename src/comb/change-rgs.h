...
// See OEIS sequence A000522.
{
 public:
  ...
  change_rgs(const change_rgs &) = delete;
  change_rgs &operator=(const change_rgs &) = delete;
 ...
 public:
  ...
  // Return position of leftmost change, zero with last.
  {
    ...
    m_[j] = nc;
    for (ulong z = j + 1; z < n_; ++z) {
      nc += (a_[z - 1] != a_[z]);
      m_[z] = nc;
    }

    //        ulong nc = m_[j-1] + ( a_[j] == a_[j-1] );  // non-changes:
    //        A000110 m_[j] = nc; for (ulong z=j+1; z<n_; ++z) {
    //        nc+=(a_[z-1]==a_[z]);  m_[z] = nc; }
