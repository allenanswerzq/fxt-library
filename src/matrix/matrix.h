...
// Deprecated, usage is strongly discouraged.
{
 ...
 public:
  matrix(ulong n) : array2d<Type>(n, n) { ; }

  matrix(const matrix<Type> &m) : array2d<Type>(m.nr_, m.nc_) { *this = m; }
  ...
  void transpose() { ::transpose_square(rowp_, nr_); }
  ...
  matrix<Type> operator*(const matrix<Type> &B) const {
    ...
    for (ulong r = 0; r < nr_; ++r) {
      for (ulong c = 0; c < nc_; ++c) {
        Type s = 0;
        for (ulong k = 0; k < nc_; ++k) s += A[r][k] * B[k][c];
        R[r][c] = s;
      }
    }
    return R;
