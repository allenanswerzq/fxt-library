...
// 3-dimensional vector with basic operations.
{
 ...
 public:
  ...
  vector3d &operator*=(const Type &s) {
    x_ *= s, y_ *= s;
    z_ *= s;
    return *this;
  }
...
template <typename Type>
inline vector3d<Type> outer_left(const vector3d<Type> &V,
                                 const vector3d<Type> &W)
// Return cross product for left handed coordinates.
{
  return vector3d<Type>(V.z() * W.y() - V.y() * W.z(),
                        V.x() * W.z() - V.z() * W.x(),
                        V.y() * W.x() - V.x() * W.y());
}
// -------------------------
...
template <typename Type>
inline vector3d<Type> outer_right(const vector3d<Type> &V,
                                  const vector3d<Type> &W)
// Return cross product for right handed coordinates.
{
  return vector3d<Type>(V.y() * W.z() - V.z() * W.y(),
                        V.z() * W.x() - V.x() * W.z(),
                        V.x() * W.y() - V.y() * W.x());
}
// -------------------------
