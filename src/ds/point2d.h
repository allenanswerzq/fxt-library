...
// Point in 2 dimensions with basic operations.
{
 ...
 public:
  explicit constexpr point2d(const Type &tx, const Type &ty) : x_(tx), y_(ty) {
    ;
  }

  // Concession to programmer's reality:
  explicit constexpr point2d(const vec2d &V) : x_(V.x()), y_(V.y()) { ; }
...
inline point2d<double> mid_point(const point2d<double> &P,
                                 const point2d<double> &Q) {
  return point2d<double>(0.5 * (P.x() + Q.x()), 0.5 * (P.y() + Q.y()));
}
// -------------------------

inline point2d<float> mid_point(const point2d<float> &P,
                                const point2d<float> &Q) {
  return point2d<float>(0.5f * (P.x() + Q.x()), 0.5f * (P.y() + Q.y()));
}
// -------------------------

inline point2d<long double> mid_point(const point2d<long double> &P,
                                      const point2d<long double> &Q) {
