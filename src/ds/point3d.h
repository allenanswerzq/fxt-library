...
// Point in 3 dimensions with basic operations.
{
 ...
 public:
  ...
  explicit constexpr point3d(const vec3d &V) : x_(V.x()), y_(V.y()), z_(V.z()) {
    ;
  }
...
inline point3d<double> mid_point(const point3d<double> &P,
                                 const point3d<double> &Q) {
  return point3d<double>(0.5 * (P.x() + Q.x()), 0.5 * (P.y() + Q.y()),
                         0.5 * (P.z() + Q.z()));
}
// -------------------------

inline point3d<float> mid_point(const point3d<float> &P,
                                const point3d<float> &Q) {
  return point3d<float>(0.5f * (P.x() + Q.x()), 0.5f * (P.y() + Q.y()),
                        0.5f * (P.z() + Q.z()));
}
// -------------------------

inline point3d<long double> mid_point(const point3d<long double> &P,
                                      const point3d<long double> &Q) {
  return point3d<long double>(0.5L * (P.x() + Q.x()), 0.5L * (P.y() + Q.y()),
                              0.5L * (P.z() + Q.z()));
}
// -------------------------
...
inline point3d<Type> mid_point(const point3d<Type> &P, const point3d<Type> &Q) {
  ...
  return point3d<Type>((P.x() + Q.x()) / 2, (P.y() + Q.y()) / 2,
                       (P.z() + Q.z()) / 2);
}
// -------------------------
