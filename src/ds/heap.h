...
// k are heaps, move the element z[k] (down) until the tree below node k is a
// heap. Data expected in z[1,2,...,n].
{
  ulong m = k;  // index of max of k, left(k), and right(k)

  const ulong l = (k << 1);  // left(k);
  if ((l <= n) && (z[l] > z[k]))
    m = l;  // left child (exists and) greater than k

  const ulong r = (k << 1) + 1;  // right(k);
  if ((r <= n) && (z[r] > z[m]))
    m = r;  // right child (exists and) greater than max(k,l)
