...
// n must be <= BITS_PER_LONG.
{
 ...
 public:
  ...
  ulong set_normal(ulong r) {
    v_ = n2p(r);
    return v_;
  }

  ulong order() const { return gf2n_order(v_, c_, h_, mfact_); }
  ...
  ulong trace() const { return gf2n_fast_trace(v_, tv_); }
  ...
  inline GF2n &operator=(const GF2n &f) {
    v_ = f.v_;
    return *this;
  }
  inline GF2n &operator=(ulong i) {
    (*this) = GF2n(i);
    return *this;
  }
