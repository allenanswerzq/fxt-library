...
const char* GF2n::pc_ =
    nullptr;  // chars to print for zero and one: e.g.  "01" or ".1"

// fixme: the following three require global constructors (and global
// destructors):
...
// If trustme is set, then the asserts are omitted.
{
  ...
  if (0 == c_) {
    if (normalq)
      c_ = highbit_normal_primpoly[n_];
    else
      c_ = minweight_primpoly[n_];

    //        g_ = 2UL;  // =='x' (comment out to always search for max order
    //        element)
  } else  // polynomial supplied
  {
    // do not accept reducible polynomials:
    //        if ( n_ < BITS_PER_LONG )  // test only works for polynomials that
    //        fit into words
    {
      if (!trustme) {
#ifdef INIT_ASSERT
...
GF2n::print_info(int level /*=0*/) {
  ...
  print_bin("  h = ", h_, pn, pc_);
  cout << " (aux. bit-mask)" << endl;
  print_bin("  g = ", g_, pn, pc_);
  cout << " (element of maximal order)" << endl;
  print_bin("  tv= ", tv_, pn, pc_);
  cout << " (traces of x^i)" << endl;
  //    {  // TEST:
  //        ulong qq = gf2n_trace_vector_x(c_, n_);
  //        print_bin(" ?tv= ", qq, pn, pc_);  cout << " (traces of x^i)" <<
  //        endl; jjassert( qq == tv_ );
  //    }

  //    print_bin("tr0e= ", tr0e.v_, pn, pc_);  cout << " (element with
  //    trace=0)" << endl;
  print_bin("tr1e= ", tr1e.v_, pn, pc_);
  cout << " (element with trace=1)" << endl;
