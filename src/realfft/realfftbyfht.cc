...
//
{
  ...
  if (is > 0)
    ...
    for (ulong i = 1, j = n - 1; i < j; i++, j--) sumdiff05_r(f[i], f[j]);
}
// -------------------------
...
// Ordering of input like the output of fht_real_complex_fft()
{
  ...
  if (is > 0)
    for (ulong i = 1, j = n - 1; i < j; i++, j--) sumdiff(f[i], f[j]);
  else
    for (ulong i = 1, j = n - 1; i < j; i++, j--) diffsum(f[i], f[j]);

  fht(f, ldn);
}
// -------------------------

void realisator(double *gr, const double *gi, ulong n, int is)
...
void imaginator(const double *gr, double *gi, ulong n, int is)
// Let gr[], gi[] be the FFT of a complex sequence.
...
void separator(double *gr, double *gi, ulong n, int is)
// Let gr[], gi[] be the FFT of a complex sequence.
