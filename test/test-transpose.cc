...
#define SRC(k) \
  (((unsigned long long)(k)*nc) % n1)  // note: overflow if k*nc >= 2^64
template <typename Type>
int is_transposed(const Type *f, ulong nr, ulong nc)
...
// with set_seq(f, nr*nc, 0, 1) and then transposed
{
  ...
  for (ulong k = 0; k < n1; ++k) {
    ulong t = SRC(k);
    if (f[k] != t) {
      cout << " element # " << k << " should be == " << t << "  but equals "
           << f[k] << endl;
      return 0;
    }
  }
...
template <typename Type>
void test_print2d(const Type *f, ulong nr, ulong nc) {
  //    cout.precision(21);
...
int main(int argc, char **argv) {
  ...
  for (ulong ldn = minldn; ldn <= maxldn; ++ldn) {
    ...
    for (ulong ldnr = 0; ldnr <= ldn; ++ldnr) {
      ...
      EQUIV_REAL(acopy(ar, fr, n); acopy(ar, gr, n); transpose(gr, nr, nc);
                 transpose(gr, nc, nr););

      EQUIV_REAL(acopy(ar, fr, n); acopy(ar, gr, n);
      ...
      EQUIV_REAL(acopy(ar, fr, n); transpose(fr, nr, nc); acopy(ar, gr, n);
                 transpose(gr, nr, nc););
