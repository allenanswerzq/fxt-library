...
static inline ulong knuth_revbin64(ulong x) {
  ...
  y = (x ^ (x >> 4)) & 0x0300c0303030c303UL;
  x ^= y ^ (y << 4);
  y = (x ^ (x >> 8)) & 0x00c0300c03f0003fUL;
  x ^= y ^ (y << 8);
  y = (x ^ (x >> 20)) & 0x00000ffc00003fffUL;
  x ^= y ^ (y << 20);
  x = (x >> 34) | (x << 30);
  return (x);
}
...
//{
//    ulong z;
//    z = (x ^ (x <<  2)) & 0x4924924924924924UL;  x ^= z ^ (z >>  2);  //
//    3-runs reversed z = (x ^ (x <<  6)) & 0x70381c0e070381c0UL;  x ^= z ^ (z
//    >>  6);  // 9-runs reversed z = (x ^ (x << 18)) & 0x003fe00007fc0000UL;  x
//    ^= z ^ (z >> 18);  // 27-runs reversed
////    z = (x ^ (x << 54)) & 0xffc0000000000000UL;  x ^= z ^ (z >> 54);  //
///81-runs ???
...
static inline ulong jj3_revbin64(ulong x) {
  ulong z;
  //    z = (x ^ (x >>  2)) &
  //    0b1001001001001001001001001001001001001001001001001001001001001001UL;  x
  //    ^= z ^ (z <<  2);  // clear highest bit z = (x ^ (x >>  2)) &
  //    0x9249249249249249UL;  x ^= z ^ (z <<  2);  // clear highest bit
  ...
  const ulong m02 =
      0b0001001001001001001001001001001001001001001001001001001001001001UL;
  const ulong m06 =
      0b0000000111000000111000000111000000111000000111000000111000000111UL;
  const ulong m18 =
      0b0000000000000000000111111111000000000000000000000000000111111111UL;
  const ulong m36 =
      0b0000000000000000000000000000000000000111111111111111111111111111UL;
#endif

  z = (x ^ (x >> 2)) & m02;
  x ^= z ^ (z << 2);  // low  3 bits reversed
  z = (x ^ (x >> 6)) & m06;
  x ^= z ^ (z << 6);  // low  9 bits reversed
  z = (x ^ (x >> 18)) & m18;
  x ^= z ^ (z << 18);  // low 27 bits reversed
  z = (x ^ (x >> 36)) & m36;
  x ^= z ^ (z << 36);  // low 63 bits reversed
...
static inline uint jj3_revbin32(uint x) {
  ...
  z = (x ^ (x >> 6)) & 0x00380038U;
  x ^= z ^ (z << 6);
  z = (x ^ (x >> 12)) & 0x00070007U;
  x ^= z ^ (z << 12);
  z = (x ^ (x >> 16)) & 0x00007fffU;
  x ^= z ^ (z << 16);
#else  // same code using GCC's binary literals
  z = (x ^ (x >> 2)) & 0b00010010010010010001001001001001U;
  x ^= z ^ (z << 2);
  z = (x ^ (x >> 6)) & 0b00000000001110000000000000111000U;
  x ^= z ^ (z << 6);
  z = (x ^ (x >> 12)) & 0b00000000000001110000000000000111U;
  x ^= z ^ (z << 12);
  z = (x ^ (x >> 16)) & 0b00000000000000000111111111111111U;
  x ^= z ^ (z << 16);
#endif
...
static inline uint jj3_revbin32_4a(uint x) {
  ...
  z = (x ^ (x >> 3)) & 0x18c60c63U;
  x ^= z ^ (z << 3);
  z = (x ^ (x >> 10)) & 0x003e001fU;
  x ^= z ^ (z << 10);
  z = (x ^ (x >> 17)) & 0x00007fffU;
  x ^= z ^ (z << 17);
#else  // same code using GCC's binary literals
...
static inline uint jj3_revbin32_4b(uint x) {
  ...
  x ^= z ^ (z << 2);
  // [ 3 ] --> [ 1, 1, 1 ] and [ 4 ] --> [ 2, 2 ]  ~ [ 2,2, 1,1,1, 2,2,  2,2, 2,
  // 2,2,  2,2, 1,1,1, 2,2 ]
...
#if 0
#else  // same code using GCC's binary literals
  z = (x ^ (x >> 1)) & 0b01001001001001001001001001001001U;
  x ^= z ^ (z << 1);
  z = (x ^ (x >> 3)) & 0b00011000100011000100011000100011U;
  x ^= z ^ (z << 3);
  z = (x ^ (x >> 9)) & 0b00000000011111000000000000011111U;
  x ^= z ^ (z << 9);
  z = (x ^ (x >> 18)) & 0b00000000000000000011111111111111U;
  x ^= z ^ (z << 18);
#endif
  return x;
}
...
#if 0
#else  // same code using GCC's binary literals
  z = (x ^ (x >> 2)) & 0b00100100010010010010010010001001U;
  x ^= z ^ (z << 2);
  z = (x ^ (x >> 3)) & 0b00011100001110001110001110000111U;
  x ^= z ^ (z << 3);
  z = (x ^ (x >> 7)) & 0b00000001111110000000000000111111U;
  x ^= z ^ (z << 7);
  z = (x ^ (x >> 19)) & 0b00000000000000000001111111111111U;
  x ^= z ^ (z << 19);
#endif
  return x;
}
...
static inline ulong jj3_revbin64_5(ulong x) {
  ulong z;
  z = (x ^ (x >> 1)) & 0x4a52a5294a52a529UL;
  x ^= z ^ (z << 1);
  z = (x ^ (x >> 3)) & 0x18c60c6318c60c63UL;
  x ^= z ^ (z << 3);
  z = (x ^ (x >> 10)) & 0x003e001f003e001fUL;
  x ^= z ^ (z << 10);
  z = (x ^ (x >> 17)) & 0x00007fff00007fffUL;
  x ^= z ^ (z << 17);
  z = (x ^ (x >> 32)) & 0x00000000ffffffffUL;
  x ^= z ^ (z << 32);
  return x;
}
// -------------------------

static inline uint jj3_revbin17(uint x) {  // [ 3 1 3 3 3 1 3 ]
  uint z;
#if 0
#else  // same code using GCC's binary literals
  z = (x ^ (x >> 2)) & 0b00100010010010001;
  x ^= z ^ (z << 2);
  z = (x ^ (x >> 4)) & 0b00001110000000111;
  x ^= z ^ (z << 4);
  z = (x ^ (x >> 10)) & 0b00000000001111111;
  x ^= z ^ (z << 10);
#endif
  return x;
}
...
static inline ulong jj3_revbin40(ulong x) {  // cf. https://oeis.org/A125173
  ulong z;
#if 0
#else  // same code using GCC's binary literals
...
ulong transpose_bit_board(
    ulong x) {  // from "Hacker's delight": transpose8rS64(...) from file
                // transpose8.c
  ulong t;
...
// Based on code by Juha Jaervi
{
  ...
  //    const ulong order = 0;  // ???
  //    x = ( ct ^ (t>>1) ^ ( (ct ^ (ct>>1) ^ -(~order&1) ^ ~0UL/15*4 ) & t ) )
  //    & M5; x = ( ct ^ (t>>1) ^ ( (ct ^ (ct>>1) ^ -1UL ^ ~0UL/15UL*4 ) & t ) )
  //    & M5; x = ( ct ^ (t>>1) ^ ( (ct ^ (ct>>1) ^ 0xbbbbbbbbbbbbbbbbUL ) & t )
  //    ) & M5;
...
//#define INCR_STEP_8  ( 0x80UL << 56 | 0x40UL << 48 | 0x20UL << 40 | 0x10UL <<
//32 | 0x8UL << 24 | 0x4UL << 16 | 0x2UL << 8 | 0x1 )
#define INCR_STEP_8 0x8040201008040201UL

//#define LEQ_FUNC(x,y)  ( ( ( ( ( (y) | MSBS_STEP_8 ) - ( (x) & ~MSBS_STEP_8 )
//) ^ (x) ^ (y) ) & MSBS_STEP_8 ) >> 7 )
...
//#define ZCMP_FUNC(x)  ( ( ( (x) | ( ( (x) | MSBS_STEP_8 ) - ONES_STEP_8 ) ) &
//MSBS_STEP_8 ) >> 7 )
...
// Adaptation from code given by Sebastiano Vigna.
{
  ...
  //    byte_sums = ( byte_sums & 3 * ONES_STEP_4 ) + ( ( byte_sums >> 2 ) & 3 *
  //    ONES_STEP_4 );
  ...
  //    const ulong place = ( ( LEQ_FUNC( byte_sums, i_step_8 ) * ONES_STEP_8 )
  //    >> 53 ) & ~0x7UL;
  const ulong place =
      ((leq_func(byte_sums, i_step_8) * ONES_STEP_8) >> 53) & ~0x7UL;
  ...
  //    return  place + ( ( LEQ_FUNC( bit_sums, byte_rank_step_8 ) * ONES_STEP_8
  //    ) >> 56 );
...
void do_some_tests(ulong v) {
  ...
  t = CLH(v);
  jjassert(bit_zip(t) == bit_zip0(t));
  t = CLH(v);
  jjassert(t == bit_unzip0(bit_zip0(t)));
  ...
  t = highest_one_01edge(v);
  jjassert(bit_count(t) == bit_count_01(t));

  jjassert((lowest_one_01edge(v) & lowest_one_10edge(v)) == lowest_one(v));

  jjassert((v == 0) || (lowest_one_idx(v) ==
                        (BITS_PER_LONG - 1 - highest_one_idx(revbin(v)))));
...
#define X(r, v)                                     \
  ...
  print_bin(" r = ", r);                            \
  cout << endl;                                     \
  print_bin(" COND = ", COND, 3);                   \
  cout << endl;                                     \
  print_bin(" ld(r) = ", ld(r), 8);                 \
  cout << endl;                                     \
  print_bin(" ld(v) = ", ld(v), 8);                 \
  cout << endl;                                     \
  print_bin(" ld_eq  v r = ", ld_eq(v, r), 3);      \
  cout << endl;                                     \
  print_bin(" ld_neq v r = ", ld_neq(v, r), 3);     \
  cout << endl;                                     \
  print_bin(" ld(v)==ld(r) = ", ld(v) == ld(r), 3); \
  cout << endl;                                     \
  print_bin(" ld(v)!=ld(r) = ", ld(v) != ld(r), 3); \
  cout << endl;
#endif
...
//    jjassert( highest_one_idx(v) == (63-highest_one_idx__(v)) );
  ...
  jjassert(block_border_ones(v) ==
           (low_block_border_ones(v) ^ high_block_border_ones(v)));
  ...
  //    jjassert( (~inverse_rev_gray_code(v)) == (inverse_gray_code(v)>>1) ); //
  //    not true
