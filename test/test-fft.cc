...
int main(int argc, char **argv) {
  ...
  for (ulong r = 1; r <= rep; ++r) {
...

    {
      ...
      CHECK(fft8_dif_core_p1(gc); fft8_dit_core_m1(gc);
            multiply_val(gc, n, (Complex)(1.0 / (double)n)););
      CHECK(fft8_dit_core_m1(gc); fft8_dif_core_p1(gc);
            multiply_val(gc, n, (Complex)(1.0 / (double)n)););
      CHECK(fft8_dit_core_p1(gc); fft8_dif_core_m1(gc);
            multiply_val(gc, n, (Complex)(1.0 / (double)n)););
      CHECK(fft8_dif_core_m1(gc); fft8_dit_core_p1(gc);
            multiply_val(gc, n, (Complex)(1.0 / (double)n)););

      CHECK(fft8_dif_core_p1(gr, gi); fft8_dit_core_m1(gr, gi);
            multiply_val(gr, 2 * n, 1.0 / (double)n););
      CHECK(fft8_dit_core_m1(gr, gi); fft8_dif_core_p1(gr, gi);
            multiply_val(gr, 2 * n, 1.0 / (double)n););
      CHECK(fft8_dit_core_p1(gr, gi); fft8_dif_core_m1(gr, gi);
            multiply_val(gr, 2 * n, 1.0 / (double)n););
      CHECK(fft8_dif_core_m1(gr, gi); fft8_dit_core_p1(gr, gi);
            multiply_val(gr, 2 * n, 1.0 / (double)n););
      ...
      for (ulong i = 0; i < 9; ++i)
        ac[i] = Complex(white_noise(), white_noise());
      for (ulong i = 0; i < n; ++i) fc[i] = ac[i];
      CHECK(fft9_p1(gr, gi); fft9_m1(gr, gi);
            multiply_val(gc, n, (Complex)(1.0 / (double)n)););
      CHECK(fft9_p1(gc); fft9_m1(gc);
            multiply_val(gc, n, (Complex)(1.0 / (double)n)););
    }
    ...
    for (ulong ldn = minldn; ldn <= maxldn; ++ldn) {
      ...
      for (int is = +1; is >= -1; is -= 2) {
        ...
        acopy(ac, fc, n);
        CHECK(fht_fft(gr, gi, ldn, is); /**/ separator(gr, gi, n, is);
              multiply_val(gr, 2 * n, 1.0 / (double)n);
        ...
        acopy(ac, fc, n);
        acopy(ac, gc, n);
        EQUIV(unzip(fr, 2 * n); fht_dif(fr, ldn); fht_dif(fi, ldn);
              /**/ fht_dif(gc, ldn); unzip(gr, 2 * n););
        ...
        CHECK(easy_ordering_real_complex_fft(gr, ldn, is);
              if (n >= 4) reverse(gr + nh + 1, nh - 1););
        ...
        CHECK(fht_real_complex_fft(gr, ldn, is);
              /**/ fht_complex_real_fft(gr, ldn, is);
              multiply_val(gr, n, 1.0 / (double)n););

        CHECK(split_radix_real_complex_fft(gr, ldn, is);
              /**/ split_radix_complex_real_fft(gr, ldn, is);
              multiply_val(gr, n, 1.0 / (double)n););

        if (is > 0)
          CHECK(wrap_real_complex_fft(gr, ldn);
                /**/ wrap_complex_real_fft(gr, ldn);
                multiply_val(gr, n, 1.0 / (double)n););

        CHECK(fft_dif4_core_p1(gc, ldn); fft_dit4_core_m1(gc, ldn);
              multiply_val(gr, 2 * n, 1.0 / (double)n););
        CHECK(fft_dit4_core_p1(gc, ldn); fft_dif4_core_m1(gc, ldn);
              multiply_val(gr, 2 * n, 1.0 / (double)n););

        CHECK(split_radix_dif_fft_core(gc, ldn);
              /* is = +1 */ split_radix_dit_fft_core(gc, ldn);
              /* is = -1 */ multiply_val(gr, 2 * n, 1.0 / (double)n););
